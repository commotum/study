...
- [Introduction to Sequences](<11.1.1. Introduction to Sequences.md>)
- [Identifying Patterns](<../../../../../Elementary-&-Middle-School-Mathematics/4GM/1. Operations & Algebraic Thinking/1.2. Thinking Algebraically/Lessons/1.2.5. Identifying Patterns.md>)
...
If $a_{n + 1} =-2a_{n}$ with $a_{1} = 2$, then $a_{2} =$ $\underline{\hspace{1.5cm}}$.
...
If $a_{n + 1} = 5a_{n} - 3$ with $a_{1} = 1$, then $a_{3} =$ $\underline{\hspace{1.5cm}}$.
...
If $f(n + 1) = f(n) + 8$ with $f(1) =-6$, then $f(4) =$ $\underline{\hspace{1.5cm}}$.
...
The recursive formula for this sequence is given by$a_{n + 1} =$
$\underline{\hspace{1.5cm}}$ $\cdot a_{n}$,
$a_{1} =$
