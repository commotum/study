...
content: |-
  Find the Laplace transform $Y(s) = L{y(t)}$ for $s > \frac{1}{2}$ if

  $2y^{′} - y = 1 + t^{2}, y(0) = \frac{1}{2}$.
options:
- id: a
  content: |-
    $Y(s) = (s^{3} + s^{2} + 2)/(s^{3}(2s - 1))$
- id: b
  content: |-
    $Y(s) = (2s^{3} + 3s + 1)/(s^{3}(s + 2))$
- id: c
  content: |-
    $Y(s) = (s^{2} + 2s + 1)/(s^{2}(2s - 1))$
- id: d
  content: |-
    $Y(s) = (2s + 1)/(s^{2}(s - 1))$
- id: e
...
# MA_ANSWER_MISSING: set exactly one options[].correct=true for radio, or all correct options for checkbox
content: |-
  Find the Laplace transform $Y(s) = L{y(t)}$ for $s > 0$ if
