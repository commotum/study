...
content: |-
  Calculate the circulation $Γ$ of the vector field $F(x, y, z) = yi - xj$ along the closed curve $C$ given by $r(t) = \cos (πt)i + \sin (πt)j + (1 - t^{2})k$ for $[MATH: t ∈ [- 1, 1).]$
options:
- id: a
  content: |-
    $-1$
...
# MA_ANSWER_MISSING: set exactly one options[].correct=true for radio, or all correct options for checkbox
content: |-
  Calculate the circulation $Γ$ of the vector field $F(x, y, z) = (y - x)i$ along the closed curve $C$ given by $r(t) = (t^{2} - t)i + (t^{3} - t)j + \sin (2πt)k$ for $[MATH: t ∈ [0, 1).]$
options:
- id: a
  content: |-
    $10$
...
# MA_ANSWER_MISSING: set exactly one options[].correct=true for radio, or all correct options for checkbox
content: |-
  Let $S$ be an oriented piecewise-smooth surface bounded by the closed curve $C$ with positive orientation and parametrization $r(t) = (t^{2} - t)i + (t - t^{3})j + (2t^{2} - 2t)k$ for $[MATH: t ∈ [0, 1).]$
...
# MA_ANSWER_MISSING: set exactly one options[].correct=true for radio, or all correct options for checkbox
content: |-
  Let $S$ be an oriented piecewise-smooth surface bounded by the closed curve $C$ with positive orientation and parametrization $r(t) = \sin (πt)i + \cos (πt)j + \sin (πt)k$ for $[MATH: t ∈ [- 1, 1).]$
...
# MA_ANSWER_MISSING: set exactly one options[].correct=true for radio, or all correct options for checkbox
content: |-
  Let $C$ be the intersection of the paraboloid $z = 10 - x^{2} - y^{2}$ and the boundary of the infinite vertical column enclosed by the planes $x + y = 1$, $x = 0$, and $y = 0$. Given that the curve $C$ is oriented counter-clockwise when viewed from above and $curlF = ⟨ - 2z, 0, 0⟩$, the circulation of $F$ along $C$ can be written as

  $∮_(C)F \cdot dr =$
options:
- id: a
  content: |-
    $∫_{0}^{1}∫_{0}^{1}20 - 4 x 2 + 4 y 2dydx$
- id: b
  content: |-
    $∫_{0}^{1}∫_{0}^{1 - x}4 x 3 + 4 x y 2 - 40 xdydx$
- id: c
  content: |-
    $∫_{0}^{1}∫_{0}^{1}4 x 3 + 4 x y 2 - 40 xdydx$
- id: d
  content: |-
    $∫_{0}^{1}∫_{0}^{x}20 - 4 x 2 + 4 y 2dydx$
- id: e
  content: |-
    $∫_{0}^{1}∫_{0}^{1 - x}20 - 4 x 2 + 4 y 2dydx$
```
