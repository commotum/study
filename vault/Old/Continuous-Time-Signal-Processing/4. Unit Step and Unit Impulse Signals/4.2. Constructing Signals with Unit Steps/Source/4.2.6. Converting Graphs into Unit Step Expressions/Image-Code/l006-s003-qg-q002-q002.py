...
def main():
    ...
    ax.fill_between([0, 5], [-3, -3], [0, 0], color=SIGNAL_COLOR, alpha=0.10, zorder=1)
    t = np.array([-1.5, 0, 0, 5, 5, 6.0], dtype=float)
    x = np.array([0, 0, -3, -3, 0, 0], dtype=float)
    plot_signal(ax, t, x)
    ax.text(2.5, -3.14, '$-3$', fontsize=ANNOTATION_SIZE, ha='center', va='top', color=LABEL_COLOR, clip_on=False)
    draw_duration_bracket(ax, 0, 5, y=-0.22)
    save_figure(fig, Path(r'/home/jake/Developer/MA/PIPELINE/Electrical-and-Computer-Engineering/2-Build-Lessons/4-Image-Generation/1-Outputs/Continuous-Time-Signal-Processing/4.2--constructing-signals-with-unit-steps-Images/images/l006-s003-qg-q002-q002.png'))
    plt.close(fig)
