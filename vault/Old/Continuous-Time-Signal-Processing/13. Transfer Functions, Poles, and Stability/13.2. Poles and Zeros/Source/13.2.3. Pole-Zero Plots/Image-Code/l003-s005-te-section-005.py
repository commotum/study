...
def main():
    ...
    setup_axes(ax, xlim=xlim, ylim=ylim, xticks=[-3, -2, -1, 1, 2], yticks=[-1.5, -1.0, -0.5, 0.5, 1.0, 1.5])
    numerator_coeffs = np.array([1, 2, -1, 0], dtype=float)
    denominator_coeffs = np.array([1, 1, -4, 4], dtype=float)
    zero_roots = np.roots(numerator_coeffs)
    pole_roots = np.roots(denominator_coeffs)
    for z in zero_roots:
        draw_zero(ax, float(np.real(z)), float(np.imag(z)))
    for p in pole_roots:
        draw_pole(ax, float(np.real(p)), float(np.imag(p)))
    save_figure(fig, Path('/home/jake/Developer/MA/PIPELINE/Electrical-and-Computer-Engineering/2-Build-Lessons/4-Image-Generation/1-Outputs/Continuous-Time-Signal-Processing/13.2--poles-and-zeros-Images/images/l003-s005-te-section-005.png'))
