...
OUTPUT = Path("/home/jake/Developer/MA/PIPELINE/Electrical-and-Computer-Engineering/2-Build-Lessons/4-Image-Generation/1-Outputs/Continuous-Time-Signal-Processing/13.3--stability-of-lti-systems-Images/images/l001-s003-te-section-003-graph.png")
...
plot_signal(ax, np.array([-2.3, -0.5]), np.array([0.0, 0.0]))
plot_signal(ax, np.array([-0.5, -0.5, -0.25, -0.25]), np.array([0.0, 0.45, 0.45, 0.0]))
plot_signal(ax, np.array([-0.25, 0.0]), np.array([0.0, 0.0]))
t_tail = np.linspace(0.0, 3.8, 300)
y_tail = 0.95 * np.exp(-0.95 * t_tail)
