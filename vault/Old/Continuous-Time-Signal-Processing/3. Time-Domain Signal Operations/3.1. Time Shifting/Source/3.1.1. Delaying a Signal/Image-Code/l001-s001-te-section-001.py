...
OUTPUT_PATH = Path(r"/home/jake/Developer/MA/PIPELINE/Electrical-and-Computer-Engineering/2-Build-Lessons/4-Image-Generation/1-Outputs/Continuous-Time-Signal-Processing/3.1--time-shifting-Images/images/l001-s001-te-section-001.png")
...
def main():
    ...
    draw_triangle_signal(ax_l, *(-2.0, 0.0, 2.0, 2.0))
    draw_triangle_signal(ax_r, *(0.0, 2.0, 4.0, 2.0))
    label_point(ax_l, -2.0, 0.0, '$a$', dx=-0.08, dy=-0.3, ha='right', va='top')
    label_point(ax_l, 0.0, 2.0, '$t_0$', dx=0.0, dy=0.28, ha='center', va='bottom')
    label_point(ax_l, 2.0, 0.0, '$b$', dx=0.08, dy=-0.3, ha='left', va='top')
    label_point(ax_r, 0.0, 0.0, '$a+T$', dx=-0.08, dy=-0.3, ha='right', va='top')
    label_point(ax_r, 2.0, 2.0, '$t_0+T$', dx=0.0, dy=0.28, ha='center', va='bottom')
    label_point(ax_r, 4.0, 0.0, '$b+T$', dx=0.08, dy=-0.3, ha='left', va='top')

    fig.canvas.draw()

    draw_cross_panel_arrow(fig, ax_l, ax_r, 0.0, 2.0, 2.0, label='$T$', label_offset=(0.0, 0.014))
