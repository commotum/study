...
axis helpers, and generalized-exponential utilities for EE01-M02-04,
Generalized Exponential Signals. Later topic workers can copy or adapt these
...
UNIT_NUMBER = 2
UNIT_NAME = "Unit 2"
MODULE_ID = "EE01-M02-04"
MODULE_NUMBER = "2.4"
MODULE_NAME = "Generalized Exponential Signals"
...
def real_exponential(t, rate=0.0, amplitude=1.0):
    """Return a real exponential amplitude * exp(rate * t)."""
...
def causal_exponential(t, alpha=1.0, amplitude=1.0, t0=0.0):
    ...
    t = np.asarray(t, dtype=float)
    return amplitude * np.exp(-alpha * (t - t0)) * unit_step(t - t0)
...
def complex_exponential(t, alpha=0.0, beta=0.0, amplitude=1.0, phase=0.0):
    """Return amplitude * exp((alpha + j*beta) * t + j*phase)."""

    t = np.asarray(t, dtype=float)
    return amplitude * np.exp((alpha + 1j * beta) * t + 1j * phase)


def generalized_exponential(t, alpha=0.0, beta=0.0, amplitude=1.0, phase=0.0):
    """Alias for a generalized complex exponential."""

    return complex_exponential(t, alpha=alpha, beta=beta, amplitude=amplitude, phase=phase)


def damped_cosine(t, alpha=0.0, omega=0.0, amplitude=1.0, phase=0.0):
    """Return amplitude * exp(alpha * t) * cos(omega * t + phase)."""

    t = np.asarray(t, dtype=float)
    return amplitude * np.exp(alpha * t) * np.cos(omega * t + phase)


def damped_sine(t, alpha=0.0, omega=0.0, amplitude=1.0, phase=0.0):
    """Return amplitude * exp(alpha * t) * sin(omega * t + phase)."""

    t = np.asarray(t, dtype=float)
    return amplitude * np.exp(alpha * t) * np.sin(omega * t + phase)
