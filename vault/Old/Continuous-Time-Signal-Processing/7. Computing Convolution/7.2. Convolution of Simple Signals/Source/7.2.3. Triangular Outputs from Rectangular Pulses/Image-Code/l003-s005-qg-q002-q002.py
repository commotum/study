...
def main():
    ...
    image_output_path = Path(r"/home/jake/Developer/MA/PIPELINE/Electrical-and-Computer-Engineering/2-Build-Lessons/4-Image-Generation/1-Outputs/Continuous-Time-Signal-Processing/7.2--convolution-of-simple-signals-Images/images/l003-s005-qg-q002-q002.png")
    image_output_path.parent.mkdir(parents=True, exist_ok=True)

    fig = plt.figure(figsize=(9.8, 7.3), dpi=DPI)
    gs = fig.add_gridspec(2, 1, height_ratios=[1.12, 1.00], hspace=0.26)
    ...
    draw_pulse_box(ax_inputs, 0, 4, height=2, baseline=0, fill_alpha=0.10)
    draw_pulse_box(ax_inputs, 2.0, 6.0, height=3, baseline=0, fill_alpha=0.05)
    overlap_left = max(0, 2.0)
    overlap_right = min(4, 6.0)
    if overlap_right <= overlap_left:
        overlap_right = overlap_left + 0.05
    ax_inputs.axvspan(overlap_left, overlap_right, color=SIGNAL_COLOR, alpha=0.18, zorder=1)
    ax_inputs.text(0.12, 2.13, r"$x(\tau)$", fontsize=15, color=LABEL_COLOR, ha="left", va="bottom")
    ax_inputs.text(2.0 - 0.06, 3.13, r"$h(t-\tau)$", fontsize=15, color=LABEL_COLOR, ha="left", va="bottom")
    ax_inputs.text(0, -0.19, r"$0$", fontsize=15, color=TICK_LABEL_COLOR, ha="center", va="top")
    ax_inputs.text(4, -0.19, r"$4$", fontsize=15, color=TICK_LABEL_COLOR, ha="center", va="top")
    ax_inputs.text(2.0, -0.19, r"$t-4$", fontsize=15, color=TICK_LABEL_COLOR, ha="center", va="top")
    ax_inputs.text(6.0, -0.19, r"$t$", fontsize=15, color=TICK_LABEL_COLOR, ha="center", va="top")
    ax_inputs.text(0.5, 1.03, r"Complete triangular output for width-4 pulses", transform=ax_inputs.transAxes, fontsize=15, color=LABEL_COLOR, ha="center", va="bottom")
    ...
    draw_triangle_output(ax_output, 0, 4, 8, 1.0)
    ax_output.text(4, 1.08, r"peak", fontsize=13, color=ANNOTATION_COLOR, ha="center", va="bottom")
    ax_output.text(0.2, 0.98, r"t < 0", fontsize=12, color=ANNOTATION_COLOR, ha="left", va="bottom")
    ax_output.text(7.9, 0.98, r"t > 8", fontsize=12, color=ANNOTATION_COLOR, ha="right", va="bottom")
    ax_output.text(4, 1.12, r"peak at t = 4", fontsize=13, color=LABEL_COLOR, ha="center", va="bottom")
    ax_output.text(2.0, 0.50, r"rising branch", fontsize=12.5, color=LABEL_COLOR, ha="center", va="center")
    ax_output.text(6.0, 0.50, r"falling branch", fontsize=12.5, color=LABEL_COLOR, ha="center", va="center")
