...
def main():
    ...
    image_output_path = Path(r"/home/jake/Developer/MA/PIPELINE/Electrical-and-Computer-Engineering/2-Build-Lessons/4-Image-Generation/1-Outputs/Continuous-Time-Signal-Processing/7.2--convolution-of-simple-signals-Images/images/l003-s002-qg-q001-q001.png")
    image_output_path.parent.mkdir(parents=True, exist_ok=True)

    fig = plt.figure(figsize=(9.2, 6.7), dpi=DPI)
    gs = fig.add_gridspec(2, 1, height_ratios=[1.35, 0.60], hspace=0.35)
    ...
    draw_pulse_box(ax_main, 0, 3, height=1, baseline=0, fill_alpha=0.10)
    draw_pulse_box(ax_main, 1.5, 4.5, height=1, baseline=0, fill_alpha=0.05)
    ...
    ax_main.text(0.10, 1.16, r"$x(\tau)$", fontsize=16, color=LABEL_COLOR, ha="left", va="bottom")
    ax_main.text(1.5 - 0.08, 1.16, r"$h(t-\tau)$", fontsize=16, color=LABEL_COLOR, ha="left", va="bottom")
    ax_main.text(0, -0.17, r"$0$", fontsize=15, color=TICK_LABEL_COLOR, ha="center", va="top")
    ax_main.text(3, -0.17, r"$3$", fontsize=15, color=TICK_LABEL_COLOR, ha="center", va="top")
    ax_main.text(1.5, -0.17, r"$t-3$", fontsize=15, color=TICK_LABEL_COLOR, ha="center", va="top")
    ax_main.text(4.5, -0.17, r"$t$", fontsize=15, color=TICK_LABEL_COLOR, ha="center", va="top")
    ax_main.text(0.5, 1.03, r"Locating the time regions for width-3 rectangular pulses", transform=ax_main.transAxes, fontsize=15, color=LABEL_COLOR, ha="center", va="bottom")
    ...
    draw_timeline_axis(
        ...
        6,
        markers=[0, 3, 6],
        marker_labels=[(0, r"first contact"), (3, r"maximum overlap"), (6, r"last contact")],
        label_y=-0.22,
    )
