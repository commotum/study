...
def main():
    ...
    image_output_path = Path(r"/home/jake/Developer/MA/PIPELINE/Electrical-and-Computer-Engineering/2-Build-Lessons/4-Image-Generation/1-Outputs/Continuous-Time-Signal-Processing/7.2--convolution-of-simple-signals-Images/images/l003-s005-qg-q001-q001.png")
    ...
    draw_pulse_box(ax_inputs, 0, 3, height=1, baseline=0, fill_alpha=0.10)
    draw_pulse_box(ax_inputs, 1.5, 4.5, height=5, baseline=0, fill_alpha=0.05)
    overlap_left = max(0, 1.5)
    overlap_right = min(3, 4.5)
    if overlap_right <= overlap_left:
        overlap_right = overlap_left + 0.05
    ax_inputs.axvspan(overlap_left, overlap_right, color=SIGNAL_COLOR, alpha=0.18, zorder=1)
    ax_inputs.text(0.12, 1.13, r"$x(\tau)$", fontsize=15, color=LABEL_COLOR, ha="left", va="bottom")
    ax_inputs.text(1.5 - 0.06, 5.13, r"$h(t-\tau)$", fontsize=15, color=LABEL_COLOR, ha="left", va="bottom")
    ax_inputs.text(0, -0.19, r"$0$", fontsize=15, color=TICK_LABEL_COLOR, ha="center", va="top")
    ax_inputs.text(3, -0.19, r"$3$", fontsize=15, color=TICK_LABEL_COLOR, ha="center", va="top")
    ax_inputs.text(1.5, -0.19, r"$t-3$", fontsize=15, color=TICK_LABEL_COLOR, ha="center", va="top")
    ax_inputs.text(4.5, -0.19, r"$t$", fontsize=15, color=TICK_LABEL_COLOR, ha="center", va="top")
    ax_inputs.text(0.5, 1.03, r"Complete triangular output for width-3 pulses", transform=ax_inputs.transAxes, fontsize=15, color=LABEL_COLOR, ha="center", va="bottom")
    ...
    draw_triangle_output(ax_output, 0, 3, 6, 1.0)
    ax_output.text(3, 1.08, r"peak", fontsize=13, color=ANNOTATION_COLOR, ha="center", va="bottom")
    ax_output.text(0.2, 0.98, r"t < 0", fontsize=12, color=ANNOTATION_COLOR, ha="left", va="bottom")
    ax_output.text(5.9, 0.98, r"t > 6", fontsize=12, color=ANNOTATION_COLOR, ha="right", va="bottom")
    ax_output.text(3, 1.12, r"peak at t = 3", fontsize=13, color=LABEL_COLOR, ha="center", va="bottom")
    ax_output.text(1.5, 0.50, r"rising branch", fontsize=12.5, color=LABEL_COLOR, ha="center", va="center")
    ax_output.text(4.5, 0.50, r"falling branch", fontsize=12.5, color=LABEL_COLOR, ha="center", va="center")
