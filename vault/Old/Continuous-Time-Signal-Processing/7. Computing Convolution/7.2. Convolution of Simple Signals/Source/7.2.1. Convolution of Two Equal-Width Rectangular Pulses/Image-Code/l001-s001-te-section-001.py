...
def main():
    configure()
    fig, ax = plt.subplots(figsize=(9.5, 5.9), dpi=160)
    setup_axes(
        ax,
        xlim=(-1.6, 4.8),
        ylim=(-0.55, 2.2),
        xticks=[(-1, '$t-T$'), (0, '$0$'), (3, '$t$'), (4, '$T$')],
        yticks=[],
        xlabel=TAU,
        ylabel='',
        show_origin=False,
    ...
    draw_span(ax, 0, 3)
    draw_support(ax, 0, 4, y=1.18, height=0.42, label='$x(\\tau)$')
    draw_support(ax, -1, 3, y=0.34, height=0.42, label='$h(t-\\tau)$')
    draw_mark(ax, 0, y0=0.0, y1=1.1, label=None, label_y=None)
    draw_mark(ax, 3, y0=0.0, y1=1.1, label=None, label_y=None)
    draw_note(ax, 2.0, 1.04, '$[0,\\,T]$', ha='center', va='center', size=14)
    draw_note(ax, 1.0, 0.22, '$[t-T,\\,t]$', ha='center', va='center', size=14)
    draw_note(ax, 1.5, 0.94, '$L(t)$', ha='center', va='center', size=15)
    save(fig, '/home/jake/Developer/MA/PIPELINE/Electrical-and-Computer-Engineering/2-Build-Lessons/4-Image-Generation/1-Outputs/Continuous-Time-Signal-Processing/7.2--convolution-of-simple-signals-Images/images/l001-s001-te-section-001.png')
