...
ax.axvspan(-1.4, -1.0, ymin=0.62, ymax=0.82, color="#b0b0b0", alpha=0.28, zorder=1)
ax.axvspan(-1.0, 0.0, ymin=0.38, ymax=0.56, color=SIGNAL_COLOR, alpha=0.16, zorder=1)
ax.axvspan(0.0, 1.0, ymin=0.18, ymax=0.36, color=SIGNAL_COLOR, alpha=0.16, zorder=1)
ax.axvspan(1.0, 2.0, ymin=0.62, ymax=0.82, color="#b0b0b0", alpha=0.28, zorder=1)

for x in [-1.0, 0.0, 1.0, 2.0]:
    ax.plot([x, x], [-0.10, 0.92], color=GUIDE_COLOR, linewidth=1.0, linestyle=(0, (1.1, 2.4)), zorder=2)

ax.text(-1.2, 0.72, '$y(t)=0$', fontsize=16, ha="center", va="center", color=LABEL_COLOR)
ax.text((-1.0 + 0.0) / 2.0, 0.47, '$(t+1)^2/2$', fontsize=16, ha="center", va="center", color=LABEL_COLOR)
ax.text((0.0 + 1.0) / 2.0, 0.27, '$1/2$', fontsize=16, ha="center", va="center", color=LABEL_COLOR)
ax.text(2.15, 0.72, '$y(t)=0$', fontsize=16, ha="center", va="center", color=LABEL_COLOR)

ax.text(-1.0, 0.57, '$-1\\leqq t<0$', fontsize=14, ha="left", va="bottom", color=ANNOTATION_COLOR)
ax.text(0.0, 0.37, '$0\\leqq t<1$', fontsize=14, ha="left", va="bottom", color=ANNOTATION_COLOR)
ax.text(-1.2, 0.84, '$t<-1$', fontsize=14, ha="center", va="bottom", color=ANNOTATION_COLOR)
ax.text(2.15, 0.84, '$t>2$', fontsize=14, ha="center", va="bottom", color=ANNOTATION_COLOR)
ax.text((-1.4 + 2.4) / 2.0, 0.96, r"assemble into $y(t)$", fontsize=15, ha="center", va="top", color=LABEL_COLOR)

Path('/home/jake/Developer/MA/PIPELINE/Electrical-and-Computer-Engineering/2-Build-Lessons/4-Image-Generation/1-Outputs/Continuous-Time-Signal-Processing/7.4--direct-convolution-for-lti-systems-Images/images/l004-s006-qg-q002-q002.png').parent.mkdir(parents=True, exist_ok=True)
plt.tight_layout()
fig.savefig('/home/jake/Developer/MA/PIPELINE/Electrical-and-Computer-Engineering/2-Build-Lessons/4-Image-Generation/1-Outputs/Continuous-Time-Signal-Processing/7.4--direct-convolution-for-lti-systems-Images/images/l004-s006-qg-q002-q002.png', bbox_inches="tight", facecolor="white")
plt.close(fig)
