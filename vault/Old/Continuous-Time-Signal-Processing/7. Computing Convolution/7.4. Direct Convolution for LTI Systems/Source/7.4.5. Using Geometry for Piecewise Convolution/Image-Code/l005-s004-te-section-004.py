...
def main():
    ...
    # Trapezoidal product region.
    x0, x1 = 0.0, 2.25
    h_left, h_right = 2.0, 2.8
    ax.fill([x0, x1, x1, x0], [0.0, 0.0, h_right, h_left], color=SIGNAL_COLOR, alpha=0.18, zorder=3)
    ax.plot([x0, x1, x1, x0, x0], [0.0, 0.0, h_right, h_left, 0.0], color=SIGNAL_COLOR, lw=SIGNAL_LW, zorder=4)
    ax.text((x0 + x1) / 2, max(h_left, h_right) * 0.63, r"$x(	au)h(t-	au)$", fontsize=15,
            ha="center", va="center", color=SIGNAL_COLOR)

    draw_duration_bracket(ax, x0, x1, y=-0.40, label='$1$')
    draw_amplitude_bracket(ax, x0 - 0.28, 0.0, h_left)
    draw_amplitude_bracket(ax, x1 + 0.28, 0.0, h_right)
    ax.text(x0 - 0.48, h_left / 2, '$2$', fontsize=16, ha="right", va="center", color=LABEL_COLOR)
    ax.text(x1 + 0.48, h_right / 2, '$t+1$', fontsize=16, ha="left", va="center", color=LABEL_COLOR)
    ax.text((x0 + x1) / 2, -0.12, '1 <= t <= 2', fontsize=15, ha="center", va="top", color=LABEL_COLOR)
    plt.tight_layout()
    Path('/home/jake/Developer/MA/PIPELINE/Electrical-and-Computer-Engineering/2-Build-Lessons/4-Image-Generation/1-Outputs/Continuous-Time-Signal-Processing/7.4--direct-convolution-for-lti-systems-Images/images/l005-s004-te-section-004.png').parent.mkdir(parents=True, exist_ok=True)
    fig.savefig('/home/jake/Developer/MA/PIPELINE/Electrical-and-Computer-Engineering/2-Build-Lessons/4-Image-Generation/1-Outputs/Continuous-Time-Signal-Processing/7.4--direct-convolution-for-lti-systems-Images/images/l005-s004-te-section-004.png', bbox_inches="tight", facecolor="white")
    plt.close(fig)
