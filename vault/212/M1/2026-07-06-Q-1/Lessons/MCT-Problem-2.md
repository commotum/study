...
When an object rotates, a point on it sweeps around the axis: the object turns through an angle while the point travels along a circular path. The radius connects these angular and linear descriptions, and the same connection applies to their rates.
...
As a point moves around the axis, it traces an arc whose length depends on both the swept angle and the radius.
...
**Source-video example:** A second wheel has diameter $30\,\mathrm{cm}$ and angular speed $40\,\mathrm{rad/s}$. Find the tangential speed of its rim.

**Explanation**

The given length is again a diameter, so prepare the radius before using $v=r\omega$:

$$
r=\frac{30\,\mathrm{cm}}{2}=15\,\mathrm{cm}=0.15\,\mathrm m.
$$

Then

$$
v=r\omega=(0.15\,\mathrm m)(40\,\mathrm{rad/s})
=6.0\,\mathrm{m/s}.
$$

The same video continues by using this speed to calculate radial acceleration. That next decision belongs in [Radial, Tangential, and Net Acceleration](<MCT-Problem-4.md>); here the stopping point is the angular-to-linear conversion.
