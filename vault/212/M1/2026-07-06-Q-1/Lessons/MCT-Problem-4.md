...
Both formulas produce acceleration units because radians are dimensionless:
...
**Example (source video):** At one instant in the source diagram, the inward radial component points west and the tangential component points north. What direction does the net acceleration point?

**Explanation**

The two components are perpendicular. Their vector sum must point between west and north, so the net acceleration points northwest. This is the same right-triangle structure used later to calculate its magnitude.
...
Average angular acceleration is the signed change in angular velocity per elapsed time:
