...
where $F_T$ is the tension and $\mu$ is the string's mass per unit length. If the string and its tension stay the same, then $F_T$ and $\mu$ stay fixed, so the wave speed $v_{\mathrm{wave}}$ stays fixed.
...
The reusable move is: **hold $v_{\mathrm{wave}}$ fixed, then make $\lambda$ change inversely with $f$.**

| Quantity | What controls it here? | When the driving frequency doubles |
| --- | --- | --- |
| Wave speed $v_{\mathrm{wave}}$ | The string's $F_T$ and $\mu$ | Stays the same |
| Frequency $f$ | The driver | Doubles |
| Wavelength $\lambda$ | The constraint $v_{\mathrm{wave}}=f\lambda$ | Is halved |
...
The same string means $\mu$ is unchanged, and the unchanged tension means $F_T$ is unchanged. Therefore,
...
- id: p1-speed-q1-c
  content: |-
    It is unchanged because $F_T$ and $\mu$ are unchanged.
  correct: true
- id: p1-speed-q1-d
  content: |-
...
- id: p1-speed-q1-e
  content: |-
    It cannot be determined even though $F_T$ and $\mu$ are fixed.
```
...
**Example:** Waves travel along a string at $24\,\mathrm{m}/\mathrm{s}$. At $6\,\mathrm{Hz}$, the wavelength is

$$
\lambda_1=\frac{v_{\mathrm{wave}}}{f_1}
=\frac{24}{6}
=4\,\mathrm{m}.
$$
...
Because the string conditions are fixed, $v_1=v_2$. Applying $v_{\mathrm{wave}}=f\lambda$ before and after the change gives the constant-product relation

$$
f_1\lambda_1=f_2\lambda_2.
$$

Equivalently, because the speed remains $24\,\mathrm{m}/\mathrm{s}$,

$$
\lambda_2=\frac{v_{\mathrm{wave}}}{f_2}
=\frac{24}{12}
=2\,\mathrm{m}.
$$
...
content: |-
  Waves travel along a fixed string at $18\,\mathrm{m}/\mathrm{s}$. The frequency changes from $3\,\mathrm{Hz}$ to $6\,\mathrm{Hz}$ while the string conditions remain fixed. What are the new wave speed and wavelength?
options:
- id: p1-wave-equation-q1-a
  content: |-
    $36\,\mathrm{m}/\mathrm{s}$ and $6\,\mathrm{m}$
- id: p1-wave-equation-q1-b
  content: |-
    $36\,\mathrm{m}/\mathrm{s}$ and $3\,\mathrm{m}$
- id: p1-wave-equation-q1-c
  content: |-
    $18\,\mathrm{m}/\mathrm{s}$ and $6\,\mathrm{m}$
- id: p1-wave-equation-q1-d
  content: |-
    $18\,\mathrm{m}/\mathrm{s}$ and $3\,\mathrm{m}$
  correct: true
- id: p1-wave-equation-q1-e
  content: |-
    $9\,\mathrm{m}/\mathrm{s}$ and $3\,\mathrm{m}$
```
...
No. The driving frequency tells how many cycles are produced each second, but $F_T$ and $\mu$ set the propagation speed on the string. With $F_T$ and $\mu$ fixed, the speed cannot double. Instead, twice as many cycles must fit into the distance traveled each second, so each wavelength is half as long.
...
**Invariant:** Unchanged $F_T$ and $\mu$ mean unchanged $v_{\mathrm{wave}}$, so the product $f\lambda$ remains constant.
...
**Main trap:** A higher driving frequency does not make the wave travel faster on an unchanged string; it makes the wavelength shorter. In particular, doubling $f$ leaves $v_{\mathrm{wave}}$ unchanged and halves $\lambda$.
