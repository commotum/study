...
- Use the weight relation $F_T\approx mg$ for a stationary hanging object.
...
$$
v_{\mathrm{wave}}=\sqrt{\frac{m_b gL}{m_s}}.
$$
...
|---|---|---|
| Ball mass $m_b$ | Tension $F_T$ | $F_T\approx m_b g$ |
| String mass $m_s$ and length $L$ | Linear density $\mu$ | $\mu=m_s/L$ |
| Tension $F_T$ and density $\mu$ | Wave speed $v_{\mathrm{wave}}$ | $v_{\mathrm{wave}}=\sqrt{F_T/\mu}$ |
...
&=0.020\ \mathrm{kg}/\mathrm{m}.
\end{aligned}
$$

Thus $0.020\ \mathrm{kg}/\mathrm{m}$ means each meter of the uniform string has mass $0.020\ \mathrm{kg}$.
...
- id: a
  content: |-
    $0.030\ \mathrm{kg}/\mathrm{m}$
  correct: true
  feedback: |-
    $\mu=m_s/L=(0.12\ \mathrm{kg})/(4.0\ \mathrm{m})=0.030\ \mathrm{kg}/\mathrm{m}$.
- id: b
  content: |-
    $0.48\ \mathrm{kg}/\mathrm{m}$
  feedback: |-
    Linear density is mass divided by length, not mass multiplied by length.
- id: c
  content: |-
    $33\ \mathrm{m}/\mathrm{kg}$
  feedback: |-
    This reverses the ratio and has the reciprocal units.
- id: d
  content: |-
    $4.1\ \mathrm{kg}/\mathrm{m}$
  feedback: |-
...
**Example:** A $0.50\ \mathrm{kg}$ ball hangs at rest from the string. Approximate the string tension using $g=9.8\ \mathrm{m}/\mathrm{s}^2$.
...
\begin{aligned}
F_T&\approx m_b g\\
&=(0.50\ \mathrm{kg})(9.8\ \mathrm{m}/\mathrm{s}^2)\\
&=4.9\ \mathrm{N}.
\end{aligned}
$$

In this model, do not use the string's mass in $F_T\approx m_b g$; it has already been used to find $\mu$.

```quiz
type: radio
id: problem-1-tension-q1
content: |-
  A $0.30\ \mathrm{kg}$ ball hangs stationary from a string. Using $g=9.8\ \mathrm{m}/\mathrm{s}^2$, what tension is supplied by the ball's weight?
options:
- id: a
  content: |-
    $2.9\ \mathrm{N}$
  correct: true
  feedback: |-
    $F_T\approx m_b g=(0.30)(9.8)=2.94\ \mathrm{N}$, which is $2.9\ \mathrm{N}$ to two significant figures.
- id: b
  content: |-
    $0.031\ \mathrm{N}$
  feedback: |-
...
**Example:** A $0.50\ \mathrm{kg}$ ball hangs from a $2.0\ \mathrm{m}$ string whose mass is $0.040\ \mathrm{kg}$. Using $g=9.8\ \mathrm{m}/\mathrm{s}^2$, find the wave speed.
...
$$
F_T\approx(0.50\ \mathrm{kg})(9.8\ \mathrm{m}/\mathrm{s}^2)
=4.9\ \mathrm{N}.
$$
...
v_{\mathrm{wave}}
&=\sqrt{\frac{F_T}{\mu}}\\
&=\sqrt{\frac{4.9\ \mathrm{N}}{0.020\ \mathrm{kg}/\mathrm{m}}}\\
&=15.65\ldots\ \mathrm{m}/\mathrm{s}\\
...
content: |-
  A $0.40\ \mathrm{kg}$ ball hangs from a $2.0\ \mathrm{m}$ string whose mass is $0.050\ \mathrm{kg}$. Use $g=9.8\ \mathrm{m}/\mathrm{s}^2$. What is the wave speed to two significant figures?
options:
- id: a
  content: |-
  ...
  correct: true
  feedback: |-
    $\mu=0.050/2.0=0.025\ \mathrm{kg}/\mathrm{m}$, $F_T=0.40(9.8)=3.92\ \mathrm{N}$, and $v_{\mathrm{wave}}=\sqrt{3.92/0.025}=12.52\ldots\ \mathrm{m}/\mathrm{s}\approx13\ \mathrm{m}/\mathrm{s}$.
- id: b
  content: |-
    $160\ \mathrm{m}/\mathrm{s}$
  feedback: |-
    This is approximately $F_T/\mu$ before taking the required square root.
- id: c
  content: |-
    $8.9\ \mathrm{m}/\mathrm{s}$
  feedback: |-
...
- id: d
  ...
  feedback: |-
    This does not follow the ratio $F_T/\mu$ inside the square root.
```
...
**Example:** Show that $\sqrt{F_T/\mu}$ has units of speed.

**Explanation**

Substitute the units $[F_T]=\mathrm{N}=\mathrm{kg}\,\mathrm{m}/\mathrm{s}^2$ and $[\mu]=\mathrm{kg}/\mathrm{m}$:
...
$$
[v_{\mathrm{wave}}]=\sqrt{\mathrm{m}^2/\mathrm{s}^2}=\mathrm{m}/\mathrm{s}.
$$

Keep extra calculator digits in $\mu$, $F_T$, and $v_{\mathrm{wave}}$ until the last line. Then round using the measured givens and follow the answer field's requested format.
...
content: |-
  A wave-speed calculation gives $v_{\mathrm{wave}}=9.64\ldots\ \mathrm{m}/\mathrm{s}$. The measured givens have two significant figures, and the answer field requests meters per second as a number only. What should be entered?
options:
- id: a
  content: |-
    `9.6`
...
|---|---:|---|
| Ball mass $m_b$ | $0.26\ \mathrm{kg}$ | Sets $F_T\approx m_b g$ |
...
=0.02743\ldots\ \mathrm{kg}/\mathrm{m}.
$$
...
v_{\mathrm{wave}}
&=\sqrt{\frac{F_T}{\mu}}\\
&=\sqrt{\frac{(0.26\ \mathrm{kg})(9.81\ \mathrm{m}/\mathrm{s}^2)}{0.02743\ldots\ \mathrm{kg}/\mathrm{m}}}\\
&=9.64\ldots\ \mathrm{m}/\mathrm{s}.
\end{aligned}
$$

The instruction to disregard reflections means only the propagating wave speed is needed. The measured givens have two significant figures, so $v_{\mathrm{wave}}=9.6\ \mathrm{m}/\mathrm{s}$.
...
- id: a
  ...
  correct: true
  feedback: |-
    $\mu=0.096/3.5=0.02743\ldots\ \mathrm{kg}/\mathrm{m}$ and $F_T\approx(0.26)(9.81)\ \mathrm{N}$, so $v_{\mathrm{wave}}=\sqrt{F_T/\mu}=9.64\ldots\ \mathrm{m}/\mathrm{s}$. To two significant figures, enter `9.6`.
- id: b
  content: |-
    `93`
  feedback: |-
    This is approximately $F_T/\mu$ before taking the square root.
- id: c
  content: |-
    `5.2`
  feedback: |-
...
2. Approximate the tension from the ball: $F_T\approx m_b g$.
3. Calculate $v_{\mathrm{wave}}=\sqrt{F_T/\mu}=\sqrt{m_b gL/m_s}$.
4. Check that the units reduce to $\mathrm{m}/\mathrm{s}$.
5. Keep full precision until the end, then round and use the requested answer form.

The main trap is swapping the two masses: the **string mass** belongs in $\mu$, while the **ball mass** belongs in $F_T$.
