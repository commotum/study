...
- Distinguish source frequency $f_s$ from observed frequency $f'$.
...
**Recognition cue:** The observer is stationary, the source is moving away, and the heard frequency satisfies $f'<f_s$. Those three facts select the receding moving-source formula and make $v_s$ the target variable.
...
Because $f'<f_s$ for a receding source, the ratio $f_s/f'$ is greater than $1$, so this formula gives a positive speed.
...
content: |-
  A source moves away from a stationary observer at speed $v_s$. Which formula correctly gives the observed frequency $f'$?
options:
- id: a
  content: |-
    $f'=f_s\dfrac{v}{v+v_s}$
  correct: true
  feedback: |-
    Correct. The larger denominator makes $f'<f_s$, as required for a receding source.
- id: b
  content: |-
    $f'=f_s\dfrac{v}{v-v_s}$
  feedback: |-
...
**Example:** Solve $f'=f_s\dfrac{v}{v+v_s}$ for $v_s$.

**Explanation**

Treat $f'$, $f_s$, and $v$ as constants while isolating $v_s$. First clear the denominator by multiplying both sides by $v+v_s$:
...
Now distribute, subtract $f'v$, and divide by $f'$:
...
- id: a
  content: |-
    $v_s=v\left(\dfrac{f_s}{f'}-1\right)$
  correct: true
  feedback: |-
    Correct. Since $f_s/f'>1$ for a receding source, this gives a positive source speed.
- id: b
  content: |-
    $v_s=v\left(\dfrac{f'}{f_s}-1\right)$
  feedback: |-
...
- id: d
  content: |-
    $v_s=v\left(1-\dfrac{f_s}{f'}\right)$
  feedback: |-
...
**Example:** A source emits $30\ \mathrm{kHz}$ while moving away from a stationary observer, who hears $24\ \mathrm{kHz}$. Use $v=340\ \mathrm{m/s}$ to find the source speed.
...
$$
\frac{f_s}{f'}=\frac{30\ \mathrm{kHz}}{24\ \mathrm{kHz}}=1.25.
$$
...
content: |-
  A source emits $24\ \mathrm{kHz}$ while moving away from a stationary observer, who hears $18\ \mathrm{kHz}$. Use $v=342\ \mathrm{m/s}$. What is the source speed?
options:
- id: a
  content: |-
    $28.5\ \mathrm{m/s}$
...
- id: c
  ...
  correct: true
  feedback: |-
    Correct. $342(24/18-1)=342(1/3)=114\ \mathrm{m/s}$.
- id: d
  content: |-
    $342\ \mathrm{m/s}$
  feedback: |-
...
- id: e
  content: |-
    $456\ \mathrm{m/s}$
  feedback: |-
    This evaluates $v(f_s/f')$ but omits the $-v$ required when isolating $v_s$.
```
...
Using $343\ \mathrm{m/s}$ for the speed of sound, organize the quantities by role:

| Quantity | Value | Role |
|---|---:|---|
| emitted frequency | $f_s=25\ \mathrm{kHz}$ | numerator of the frequency ratio |
| observed frequency | $f'=20\ \mathrm{kHz}$ | denominator of the frequency ratio |
| speed of sound | $v=343\ \mathrm{m/s}$ | known propagation speed |
...
The frequencies have two significant figures, so the result is $86\ \mathrm{m/s}$. The source answer form is: **Using $343\ \mathrm{m/s}$ for the speed of sound, the bat's speed in meters per second is a number-only entry.** The correct entry is $86$.

Because the original prompt does not specify the speed of sound, using $340\ \mathrm{m/s}$ would instead give $85\ \mathrm{m/s}$. The source therefore disables exact string grading.

```quiz
type: radio
id: p7-source-check
content: |-
  A bat chirps at $25\ \mathrm{kHz}$ while flying away from you. How fast must it fly for you to hear $20\ \mathrm{kHz}$? Using $343\ \mathrm{m/s}$ for the speed of sound, which number-only entry gives the bat's speed in meters per second?
options:
- id: a
  content: |-
    $69$
  feedback: |-
    This uses the fractional frequency decrease $1-f'/f_s$ instead of the isolated Doppler expression.
- id: b
  content: |-
    $85$
  feedback: |-
    This is the rounded result if $340\ \mathrm{m/s}$ is assumed; the question explicitly asks you to use $343\ \mathrm{m/s}$.
- id: c
  content: |-
    $86$
  correct: true
  feedback: |-
    Correct. The unrounded value is $85.75\ \mathrm{m/s}$, which rounds to $86\ \mathrm{m/s}$.
- id: d
  content: |-
    $343$
  feedback: |-
...
- id: e
  content: |-
    $429$
  feedback: |-
    This computes $v(f_s/f')$ but forgets to subtract $v$.
```
...
1. Check that $f'<f_s$.
2. Use $f'=f_s\dfrac{v}{v+v_s}$.
3. Isolate the source speed: $v_s=v\left(\dfrac{f_s}{f'}-1\right)$.
4. Use the same units for $f_s$ and $f'$ so their ratio is unitless.
