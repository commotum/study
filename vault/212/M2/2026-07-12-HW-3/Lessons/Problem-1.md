...
| $r_\perp$ | that piece's shortest perpendicular distance to the chosen axis |
| $r_\perp^2dm$ | that piece's contribution to $I$ |
...
**Example:** A small mass element $dm$ lies near a vertical rotation axis. Its position is connected to the axis by a slanted segment of length $s$, while the shortest segment from the axis to the mass has length $r_\perp$. Which distance belongs in $I=\int r_\perp^2\,dm$?

**Explanation**

Use $r_\perp$, the shortest perpendicular distance to the axis. The element's contribution is

$$
dI=r_\perp^2\,dm.
$$

The integral then adds $dI$ over **all** mass elements in the specified object or system. A larger $r_\perp$ matters strongly because the distance is squared. The units also confirm the meaning:

$$
[I]=[r_\perp^2][m]=\text{mass}\cdot\text{length}^2.
$$
...
- id: b
  content: |-
    The perpendicular distances $r_\perp$ from the mass elements to the axis can change.
  correct: true
- id: c
  content: |-
    The exponent on $r_\perp$ changes with the axis.
- id: d
  content: |-
...
and the perpendicular distance is $r_\perp=x$. Therefore,
...
content: |-
  A thin plate has surface mass density $\sigma(x,y)$. Which substitution converts $I=\int r_\perp^2\,dm$ into an integral over the plate's geometry?
options:
- id: a
  content: |-
    $dm=\sigma(x,y)\,dA$
...
**Example:** Three point-like objects have masses $m_1,m_2,m_3$ and perpendicular distances $r_{\perp,1},r_{\perp,2},r_{\perp,3}$ from an axis. Their moment of inertia is

$$
I=m_1r_{\perp,1}^2+m_2r_{\perp,2}^2+m_3r_{\perp,3}^2
=\sum_{i=1}^{3}m_i r_{\perp,i}^2.
$$

**Explanation**

Use $I=\int r_\perp^2\,dm$ when mass is treated as continuously distributed. Use
...
$$
I\approx\sum_k r_{\perp,k}^2\Delta m_k.
$$

As the pieces become infinitesimal, this sum becomes $\int r_\perp^2\,dm$. The sum and integral are therefore two versions of the same accumulation rule, not unrelated formulas.

```quiz
type: radio
id: p1-integral-or-sum
content: |-
  A system consists of four point-like masses $m_i$ at perpendicular distances $r_{\perp,i}$ from the chosen axis. Which expression directly gives its moment of inertia?
options:
- id: a
  content: |-
    $\displaystyle I=\sum_{i=1}^{4}m_i r_{\perp,i}^2$
  correct: true
- id: b
  content: |-
    $\displaystyle I=\sum_{i=1}^{4}m_i r_{\perp,i}$
- id: c
  content: |-
    $\displaystyle I=\int_1^4 r_{\perp,i}^2\,dm_i$
- id: d
  content: |-
    $\displaystyle I=\left(\sum_{i=1}^{4}m_i\right)\left(\sum_{i=1}^{4}r_{\perp,i}\right)^2$
- id: e
  content: |-
    $\displaystyle I=\sum_{i=1}^{4}\frac{m_i}{r_{\perp,i}^2}$
```
...
**Example:** Consider these claims about $I=\int r_\perp^2\,dm$:

1. The variable $r_\perp$ represents the shortest (perpendicular) distance from the axis of rotation to the position of the infinitesimal mass element $dm$.
...
5. This formula applies to continuous objects or systems. For discrete point-like objects, use $I=\sum_{i=1}^{N}m_i r_{\perp,i}^2$, where the sum is over the $N$ objects, $m_i$ is each object's mass, and $r_{\perp,i}$ is its perpendicular distance from the axis.
...
content: |-
  Which statement does **not** correctly describe $I=\int r_\perp^2\,dm$?
options:
- id: a
  content: |-
...
- id: b
  content: |-
    The distance $r_\perp$ is measured perpendicular to the chosen axis.
- id: c
  content: |-
...
- id: e
  content: |-
    The distance $r_\perp$ is always measured from the object's center of mass, regardless of the axis.
  correct: true
```
...
**Cue:** You are asked what $I=\int r_\perp^2\,dm$ means or how its pieces relate to an object's mass distribution.

**Rule:** Add every mass contribution $r_\perp^2dm$, where $r_\perp$ is measured perpendicular to the chosen axis.

**Procedure:**

1. Identify the chosen rotation axis.
2. Measure $r_\perp$ as each mass element's shortest perpendicular distance to that axis.
3. Add $r_\perp^2dm$ over every mass element in the object or system.
4. Replace $dm$ with $\lambda\,ds$, $\sigma\,dA$, or $\rho\,dV$ when a density describes a continuous object.
5. Replace the integral with $\sum_i m_i r_{\perp,i}^2$ for discrete point-like masses.
