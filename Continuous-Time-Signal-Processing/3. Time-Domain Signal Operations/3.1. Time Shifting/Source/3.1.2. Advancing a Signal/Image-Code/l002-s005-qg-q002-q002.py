...
OUTPUT_PATH = Path('/home/jake/Developer/MA/PIPELINE/Electrical-and-Computer-Engineering/2-Build-Lessons/4-Image-Generation/1-Outputs/Continuous-Time-Signal-Processing/3.1--time-shifting-Images/images/l002-s005-qg-q002-q002.png')

def mark(ax, x, y, label, *, dx=0.0, dy=0.18, guide=False):
    if guide and abs(y) > 1e-12:
...
def build_figure():
    ...
    original_t = [1, 2, 4, 6, 8, 8, 9]
    original_x = [0, 0, 3.0, 3.0, 0, 0, 0]
    advanced_t = [-3, -2, 0, 2, 4, 4, 5]
    advanced_x = [0, 0, 3.0, 3.0, 0, 0, 0]

    plot_signal(ax, original_t, original_x, lw=SMOOTH_SIGNAL_LW, zorder=4)
    plot_signal(ax, advanced_t, advanced_x, lw=SIGNAL_LW, zorder=5)

    mark(ax, 4, 3.0, '$4$', dx=0.0, dy=0.18, guide=True)

    mark(ax, 0, 3.0, '$0$', dx=0.0, dy=0.18, guide=True)

    mark(ax, 2, 0, '$2$', dx=0.0, dy=0.14, guide=False)

    mark(ax, -2, 0, '$-2$', dx=0.0, dy=0.14, guide=False)

    draw_shift_arrow(ax, 4.0, 0.0, 3.35, label=None)

    return fig

def main():
    fig = build_figure()
    save_figure(fig, Path('/home/jake/Developer/MA/PIPELINE/Electrical-and-Computer-Engineering/2-Build-Lessons/4-Image-Generation/1-Outputs/Continuous-Time-Signal-Processing/3.1--time-shifting-Images/images/l002-s005-qg-q002-q002.png'))
