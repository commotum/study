...
def make_two_panel_ct_signal_figure(xlim, ylim, *, gap_px=72, dpi=CANONICAL_DPI):
    """Create a paired CTS figure with aligned data scaling."""
...
def data_to_fig_coords(fig, ax, x, y):
    """Convert an axes data point to figure-fraction coordinates."""

    return fig.transFigure.inverted().transform(ax.transData.transform((x, y)))


def draw_cross_panel_arrow(
    fig,
    ax_from,
    ax_to,
    x_from,
    ...
    *,
    label=None,
    label_offset=(0.0, 0.014),
    color=ANNOTATION_COLOR,
    lw=ANNOTATION_LW,
    mutation_scale=13,
):
    """Draw a rightward arrow between corresponding points in two panels."""

    p0 = data_to_fig_coords(fig, ax_from, x_from, y)
    p1 = data_to_fig_coords(fig, ax_to, x_to, y)
    ...
    if label is not None:
        fig.text(
            (p0[0] + p1[0]) / 2 + label_offset[0],
            max(p0[1], p1[1]) + label_offset[1],
            label,
            fontsize=ANNOTATION_SIZE,
            ha="center",
            va="bottom",
...
def label_point(ax, x, y, text, *, dx=0.0, dy=0.0, ha="center", va="bottom", color=LABEL_COLOR):
...
def draw_triangle_signal(
    ...
    *,
    ...
    """Draw a triangular, finite-support signal."""

    t = np.asarray([left, peak, right], dtype=float)
    x = np.asarray([0.0, height, 0.0], dtype=float)

    if shade:
        ax.axvspan(left, right, color=color, alpha=shade_alpha, zorder=1)

    plot_signal(ax, t, x, color=color, zorder=4)
    draw_closed_endpoint(ax, left, 0.0)
    draw_closed_endpoint(ax, peak, height)
    draw_closed_endpoint(ax, right, 0.0)

    return t, x


def draw_keypoint_polyline(ax, points, *, color=SIGNAL_COLOR):
    """Draw a connected polyline and mark each key point."""

    pts = np.asarray(points, dtype=float)
    plot_signal(ax, pts[:, 0], pts[:, 1], color=color, zorder=4)
    for x, y in pts:
        draw_closed_endpoint(ax, x, y)
    return pts

OUTPUT_PATH = Path(r"/home/jake/Developer/MA/PIPELINE/Electrical-and-Computer-Engineering/2-Build-Lessons/4-Image-Generation/1-Outputs/Continuous-Time-Signal-Processing/3.1--time-shifting-Images/images/l001-s005-te-section-005.png")
...
def main():
    ...
    fig, (ax_l, ax_r) = make_two_panel_ct_signal_figure(xlim, ylim)
    ...
    setup_ct_signal_axes(
        ...
        yticks=[1, 2, 3],
        x_axis_label=r"$t$",
        y_axis_label='$y(t)=x(t-T)$',
        show_origin=False,
    )

    draw_triangle_signal(ax_l, *(-1.0, 2.0, 4.0, 2.5))
    draw_triangle_signal(ax_r, *(1.0, 4.0, 6.0, 2.5))
    label_point(ax_l, -1.0, 0.0, '$-1$', dx=-0.08, dy=-0.3, ha='right', va='top')
    label_point(ax_l, 2.0, 2.5, '$2$', dx=0.0, dy=0.3, ha='center', va='bottom')
    label_point(ax_l, 4.0, 0.0, '$4$', dx=0.08, dy=-0.3, ha='left', va='top')
    label_point(ax_l, 0.4, 1.3, '$p_1$', dx=0.0, dy=0.0, ha='center', va='center')
    label_point(ax_l, 3.0, 1.3, '$p_2$', dx=0.0, dy=0.0, ha='center', va='center')
    label_point(ax_r, 1.0, 0.0, '$1$', dx=-0.08, dy=-0.3, ha='right', va='top')
    label_point(ax_r, 4.0, 2.5, '$4$', dx=0.0, dy=0.3, ha='center', va='bottom')
    label_point(ax_r, 6.0, 0.0, '$6$', dx=0.08, dy=-0.3, ha='left', va='top')
    label_point(ax_r, 2.4, 1.3, '$p_1$', dx=0.0, dy=0.0, ha='center', va='center')
    label_point(ax_r, 5.0, 1.3, '$p_2$', dx=0.0, dy=0.0, ha='center', va='center')

    fig.canvas.draw()

    draw_cross_panel_arrow(fig, ax_l, ax_r, -1.0, 0.0, 1.0, label='$2$', label_offset=(0.0, 0.014))
    draw_cross_panel_arrow(fig, ax_l, ax_r, 2.0, 2.5, 4.0, label='$2$', label_offset=(0.0, 0.014))
    draw_cross_panel_arrow(fig, ax_l, ax_r, 4.0, 0.0, 6.0, label='$2$', label_offset=(0.0, 0.014))
