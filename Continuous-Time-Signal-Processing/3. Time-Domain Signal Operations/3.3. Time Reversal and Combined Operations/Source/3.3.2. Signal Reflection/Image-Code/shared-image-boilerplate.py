...
EE01-M03-03, Time Reversal and Combined Operations. Later topic workers can
...
UNIT_NUMBER = 3
UNIT_NAME = "Unit 3"
MODULE_ID = "EE01-M03-03"
MODULE_NUMBER = "3.3"
MODULE_NAME = "Time Reversal and Combined Operations"
...
def draw_open_endpoint(ax, t0, x0, *, zorder=6):
...
def draw_closed_endpoint(ax, t0, x0, *, zorder=6):
...
def draw_transform_arrow(
    ...
    *,
    ...
    """Draw a horizontal transformation arrow used in operation diagrams."""
...
def draw_shift_arrow(
    ...
    *,
    ...
    """Alias for a horizontal shift arrow."""
...
def time_transform(t, *, scale=1.0, shift=0.0):
    """Apply an affine time transform to a time vector."""

    return scale * np.asarray(t, dtype=float) + shift


def time_shift(t, shift):
    """Shift a time vector by a constant amount."""

    return time_transform(t, scale=1.0, shift=shift)


def time_scale(t, scale):
    """Scale a time vector by a constant factor."""

    return time_transform(t, scale=scale, shift=0.0)


def time_reverse(t):
    """Reverse a time vector about the origin."""

    return time_transform(t, scale=-1.0, shift=0.0)


def reflect_time_about(t, axis=0.0):
    """Reflect a time vector about a vertical axis at t = axis."""

    return time_transform(t, scale=-1.0, shift=2.0 * axis)


def shift_signal_samples(t, x, shift):
    """Shift sampled signal values horizontally by a constant amount."""
...
def transform_signal_samples(t, x, *, scale=1.0, shift=0.0, sort=True):
    """Apply an affine time transform to sampled signal values."""

    t = np.asarray(t, dtype=float)
    x = np.asarray(x, dtype=float)
    t_new = time_transform(t, scale=scale, shift=shift)

    if not sort:
        return t_new, x

    order = np.argsort(t_new, kind="mergesort")
    return t_new[order], x[order]


def scale_signal_samples(t, x, scale, *, sort=True):
    """Scale sampled signal values in time by a constant factor."""

    return transform_signal_samples(t, x, scale=scale, shift=0.0, sort=sort)


def reverse_signal_samples(t, x, *, sort=True):
    """Reverse sampled signal values about the origin in time."""

    return transform_signal_samples(t, x, scale=-1.0, shift=0.0, sort=sort)


def reflect_signal_samples_about(t, x, axis=0.0, *, sort=True):
    """Reflect sampled signal values about a vertical axis at t = axis."""

    return transform_signal_samples(t, x, scale=-1.0, shift=2.0 * axis, sort=sort)
