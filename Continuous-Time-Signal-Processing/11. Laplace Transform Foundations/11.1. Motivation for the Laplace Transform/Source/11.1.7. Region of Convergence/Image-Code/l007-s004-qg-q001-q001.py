"""ROC shading example image for boundary sigma = -1, right half-plane.
"""
...
def mark_boundary_half_plane(ax, *, xlim, ylim, sigma0, shade_right=True):
    ymin, ymax = ylim
    shade_left = xlim[0]
    shade_right_x = xlim[1]
    if shade_right:
        ax.fill_betweenx(np.array([ymin, ymax]), np.array([sigma0, sigma0]), np.array([shade_right_x, shade_right_x]),
                        color=SIGNAL_COLOR, alpha=0.10, zorder=1)
        boundary_y = ymax - px_to_data(16)
        side_text_x = sigma0 + px_to_data(5)
        side_text = r"\sigma > "
    else:
        ax.fill_betweenx(np.array([ymin, ymax]), np.array([sigma0, sigma0]), np.array([shade_left, shade_left]),
                        color=SIGNAL_COLOR, alpha=0.10, zorder=1)
        boundary_y = ymax - px_to_data(16)
        side_text_x = sigma0 + px_to_data(5)
        side_text = r"\sigma < "
    ...
    ax.text(sigma0 + px_to_data(4), ymax - px_to_data(12), rf"$\sigma={boundary}$ (excluded)",
...
def main():
    ...
    mark_boundary_half_plane(ax, xlim=xlim, ylim=ylim, sigma0=-1, shade_right=True)

    output_path = Path('/home/jake/Developer/MA/PIPELINE/Electrical-and-Computer-Engineering/2-Build-Lessons/4-Image-Generation/1-Outputs/Continuous-Time-Signal-Processing/11.1--motivation-for-the-laplace-transform-Images/images/l007-s004-qg-q001-q001.png')
