"""ROC shading example image for section boundary sigma = -2, right half-plane."""
...
def mark_boundary_half_plane(ax, *, xlim, ylim, sigma0, shade_right=True):
    ymin, ymax = ylim
    ax.fill_betweenx(np.array([ymin, ymax]), np.array([sigma0, sigma0]), np.array([xlim[1], xlim[1]]),
...
def main():
    ...
    mark_boundary_half_plane(ax, xlim=xlim, ylim=ylim, sigma0=-2, shade_right=True)

    output_path = Path('/home/jake/Developer/MA/PIPELINE/Electrical-and-Computer-Engineering/2-Build-Lessons/4-Image-Generation/1-Outputs/Continuous-Time-Signal-Processing/11.1--motivation-for-the-laplace-transform-Images/images/l007-s004-te-section-004-roc.png')
