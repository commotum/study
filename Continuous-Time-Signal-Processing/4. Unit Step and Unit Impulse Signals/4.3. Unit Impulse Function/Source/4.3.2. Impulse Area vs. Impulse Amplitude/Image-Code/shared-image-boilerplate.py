"""Reusable matplotlib boilerplate for CTS Module EE01-M04-03 (Unit Impulse Function).

This module provides shared styling constants and helper functions for generating
continuous-time signal figures in the CTS image-generation pipeline.
"""
...
UNIT_NUMBER = 4
UNIT_NAME = "Unit 4"
MODULE_ID = "EE01-M04-03"
MODULE_NUMBER = "4.3"
MODULE_NAME = "Unit Impulse Function"


# ----------------------------------------------------------------------------
# Canonical style scale
# ----------------------------------------------------------------------------
plt.rcParams.update({
    "mathtext.fontset": "cm",
    "font.family": "serif",
...
def px_to_pt(px):
    """Convert canvas pixels at canonical DPI to typographic points."""
    return px * 72 / CANONICAL_DPI


def px_to_data(px):
    """Convert canvas pixels at canonical DPI to data units for axis-aware geometry."""
    return px / PX_PER_DATA_UNIT


def make_ct_signal_figure(xlim, ylim):
    """Create a figure and axis sized from explicit data limits.

    The resulting figure uses constrained_layout and the canonical DPI so that
    downstream constants derived from px_to_pt / px_to_data map correctly.
    """
...
def math_label(value):
    """Format numeric ticks as mathtext with compact integer display."""
...
def setup_ct_signal_axes(
    ...
    *,
    ...
    """Apply common axis/label/tick/grid treatment for continuous-time signals."""
    ax.set_xlim(*xlim)
    ax.set_ylim(*ylim)
    ...
    # Hide default frame; draw explicit axis arrows and ticks manually.
    ...
    # Axes with arrowheads on the positive direction only.
    ax.quiver(
        x_axis_start,
        0,
...
def plot_signal(ax, t, x, *, lw=SIGNAL_LW, color=SIGNAL_COLOR, zorder=4):
    """Plot standard continuous-time signal trace."""
    ax.plot(
        t,
        x,
        color=color,
...
def plot_smooth_signal(ax, t, x, *, lw=SMOOTH_SIGNAL_LW, color=SIGNAL_COLOR, zorder=4):
    """Plot smooth continuous signal curve."""
    ax.plot(
        t,
        x,
        color=color,
...
def save_figure(fig, output_path):
    ...
    fig.savefig(path, dpi=CANONICAL_DPI, bbox_inches="tight", facecolor="white")
