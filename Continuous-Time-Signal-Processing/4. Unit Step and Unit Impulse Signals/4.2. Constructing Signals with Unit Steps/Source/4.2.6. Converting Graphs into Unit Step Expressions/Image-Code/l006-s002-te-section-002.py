...
def main():
    ...
    t = np.array([-4, -2, -2, 0, 0, 3, 3, 5, 5, 6.5], dtype=float)
    x = np.array([0, 0, 2, 2, -1, -1, 3, 3, 0, 0], dtype=float)
    plot_signal(ax, t, x)
    ax.text(-3.0, 0.16, '$0$', fontsize=ANNOTATION_SIZE, ha='center', va='bottom', color=LABEL_COLOR, clip_on=False)
    ax.text(-1.0, 2.14, '$2$', fontsize=ANNOTATION_SIZE, ha='center', va='bottom', color=LABEL_COLOR, clip_on=False)
    ax.text(1.5, -1.84, '$-1$', fontsize=ANNOTATION_SIZE, ha='center', va='top', color=LABEL_COLOR, clip_on=False)
    ax.text(4.0, 3.14, '$3$', fontsize=ANNOTATION_SIZE, ha='center', va='bottom', color=LABEL_COLOR, clip_on=False)
    ax.text(5.8, 0.16, '$0$', fontsize=ANNOTATION_SIZE, ha='center', va='bottom', color=LABEL_COLOR, clip_on=False)
    save_figure(fig, Path(r'/home/jake/Developer/MA/PIPELINE/Electrical-and-Computer-Engineering/2-Build-Lessons/4-Image-Generation/1-Outputs/Continuous-Time-Signal-Processing/4.2--constructing-signals-with-unit-steps-Images/images/l006-s002-te-section-002.png'))
    plt.close(fig)
