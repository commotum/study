...
def main():
    ...
    t = np.array([-2.0, -1, -1, 1, 3, 3, 5, 5.8], dtype=float)
    x = np.array([0, 0, -2, -2, 2, 1, 1, 0], dtype=float)
    plot_signal(ax, t, x)
    ax.text(0.0, -2.14, '$-2$', fontsize=ANNOTATION_SIZE, ha='center', va='top', color=LABEL_COLOR, clip_on=False)
    ax.text(4.0, 1.14, '$1$', fontsize=ANNOTATION_SIZE, ha='center', va='bottom', color=LABEL_COLOR, clip_on=False)
    save_figure(fig, Path(r'/home/jake/Developer/MA/PIPELINE/Electrical-and-Computer-Engineering/2-Build-Lessons/4-Image-Generation/1-Outputs/Continuous-Time-Signal-Processing/4.2--constructing-signals-with-unit-steps-Images/images/l006-s006-qg-q002-q002.png'))
    plt.close(fig)
