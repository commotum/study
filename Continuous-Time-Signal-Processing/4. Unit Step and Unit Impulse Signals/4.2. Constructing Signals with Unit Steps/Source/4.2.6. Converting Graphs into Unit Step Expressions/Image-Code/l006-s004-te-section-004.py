...
def main():
    ...
    t = np.array([0, 1, 1, 3, 3, 5, 5, 6], dtype=float)
    x = np.array([0, 0, 2, 2, -1, -1, 0, 0], dtype=float)
    plot_signal(ax, t, x)
    ax.text(1.0, -0.16, '$a$', fontsize=ANNOTATION_SIZE, ha='center', va='top', color=LABEL_COLOR, clip_on=False)
    ax.text(3.0, -0.16, '$b$', fontsize=ANNOTATION_SIZE, ha='center', va='top', color=LABEL_COLOR, clip_on=False)
    ax.text(5.0, -0.16, '$c$', fontsize=ANNOTATION_SIZE, ha='center', va='top', color=LABEL_COLOR, clip_on=False)
    ax.text(2.0, 2.14, '$A$', fontsize=ANNOTATION_SIZE, ha='center', va='bottom', color=LABEL_COLOR, clip_on=False)
    ax.text(4.0, -1.14, '$B$', fontsize=ANNOTATION_SIZE, ha='center', va='top', color=LABEL_COLOR, clip_on=False)
    save_figure(fig, Path(r'/home/jake/Developer/MA/PIPELINE/Electrical-and-Computer-Engineering/2-Build-Lessons/4-Image-Generation/1-Outputs/Continuous-Time-Signal-Processing/4.2--constructing-signals-with-unit-steps-Images/images/l006-s004-te-section-004.png'))
    plt.close(fig)
