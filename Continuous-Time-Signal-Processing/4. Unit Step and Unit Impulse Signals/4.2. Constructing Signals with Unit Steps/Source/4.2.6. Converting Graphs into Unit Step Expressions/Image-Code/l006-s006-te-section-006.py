...
def main():
    ...
    t = np.array([-0.5, 0, 0, 2, 4, 4, 6, 6.5], dtype=float)
    x = np.array([0, 0, 2, 2, 0, -1, -1, 0], dtype=float)
    plot_signal(ax, t, x)
    ax.text(1.0, 2.14, '$2$', fontsize=ANNOTATION_SIZE, ha='center', va='bottom', color=LABEL_COLOR, clip_on=False)
    ax.text(5.0, -1.14, '$-1$', fontsize=ANNOTATION_SIZE, ha='center', va='top', color=LABEL_COLOR, clip_on=False)
    save_figure(fig, Path(r'/home/jake/Developer/MA/PIPELINE/Electrical-and-Computer-Engineering/2-Build-Lessons/4-Image-Generation/1-Outputs/Continuous-Time-Signal-Processing/4.2--constructing-signals-with-unit-steps-Images/images/l006-s006-te-section-006.png'))
    plt.close(fig)
