...
def main():
    ...
    ax.fill_between([1, 3], [0, 0], [1, 1], color=SIGNAL_COLOR, alpha=0.10, zorder=1)
    t = np.array([0, 1, 1, 3, 3, 4], dtype=float)
    x = np.array([0, 0, 1, 1, 0, 0], dtype=float)
    plot_signal(ax, t, x)
    ax.text(2.0, 1.16, '$A$', fontsize=ANNOTATION_SIZE, ha='center', va='bottom', color=LABEL_COLOR, clip_on=False)
    ...
    draw_duration_bracket(ax, 1, 3, y=-0.22)
    save_figure(fig, Path(r'/home/jake/Developer/MA/PIPELINE/Electrical-and-Computer-Engineering/2-Build-Lessons/4-Image-Generation/1-Outputs/Continuous-Time-Signal-Processing/4.2--constructing-signals-with-unit-steps-Images/images/l006-s001-te-section-001.png'))
    plt.close(fig)
