...
def main():
    ...
    ax.fill_between([-2, 1], [0, 0], [4, 4], color=SIGNAL_COLOR, alpha=0.10, zorder=1)
    t = np.array([-3.5, -2, -2, 1, 1, 4.0], dtype=float)
    x = np.array([0, 0, 4, 4, 0, 0], dtype=float)
    plot_signal(ax, t, x)
    ax.text(-0.5, 4.14, '$4$', fontsize=ANNOTATION_SIZE, ha='center', va='bottom', color=LABEL_COLOR, clip_on=False)
    draw_duration_bracket(ax, -2, 1, y=-0.22)
    save_figure(fig, Path(r'/home/jake/Developer/MA/PIPELINE/Electrical-and-Computer-Engineering/2-Build-Lessons/4-Image-Generation/1-Outputs/Continuous-Time-Signal-Processing/4.2--constructing-signals-with-unit-steps-Images/images/l006-s003-qg-q001-q001.png'))
    plt.close(fig)
