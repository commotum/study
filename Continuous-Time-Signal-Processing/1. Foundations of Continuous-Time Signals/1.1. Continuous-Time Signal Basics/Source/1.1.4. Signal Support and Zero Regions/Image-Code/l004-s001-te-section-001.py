...
OUTPUT_PATH = Path(
    "/home/jake/Developer/MA/PIPELINE/Electrical-and-Computer-Engineering/2-Build-Lessons/4-Image-Generation/1-Outputs/Continuous-Time-Signal-Processing/1.1--continuous-time-signal-basics-Images/images/l004-s001-te-section-001.png"
)
...
def build_figure():
    ...
    t = np.array([xlim[0], 1.0, 1.0, 4.0, 4.0, xlim[1]], dtype=float)
    x = np.array([0.0, 0.0, 2.0, 2.0, 0.0, 0.0], dtype=float)
    plot_signal(ax, t, x)

    bracket_y = 2.36
    bracket_cap = px_to_data(9)
    ax.plot([1.0, 4.0], [bracket_y, bracket_y],
            color=ANNOTATION_COLOR, linewidth=ANNOTATION_LW, zorder=5)
    ax.plot([1.0, 1.0], [bracket_y - bracket_cap, bracket_y + bracket_cap],
            color=ANNOTATION_COLOR, linewidth=ANNOTATION_LW, zorder=5)
    ax.plot([4.0, 4.0], [bracket_y - bracket_cap, bracket_y + bracket_cap],
            color=ANNOTATION_COLOR, linewidth=ANNOTATION_LW, zorder=5)
    ax.text(2.5, bracket_y + px_to_data(15), "support",
            fontsize=ANNOTATION_SIZE, ha="center", va="bottom",
            color=ANNOTATION_COLOR)

    ax.text(-1.6, -0.52, r"$t<1$",
            fontsize=ANNOTATION_SIZE, ha="center", va="top",
            color=ANNOTATION_COLOR)
    ax.text(5.15, -0.52, r"$t>4$",
            fontsize=ANNOTATION_SIZE, ha="center", va="top",
            color=ANNOTATION_COLOR)
