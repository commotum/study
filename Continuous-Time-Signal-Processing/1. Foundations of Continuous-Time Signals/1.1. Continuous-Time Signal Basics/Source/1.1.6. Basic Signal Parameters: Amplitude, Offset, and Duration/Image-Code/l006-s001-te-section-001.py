...
def draw_horizontal_bracket(ax, x0, x1, y, *, label=None, color=ANNOTATION_COLOR):
    cap = px_to_data(9)
    ax.plot([x0, x1], [y, y], color=color, linewidth=ANNOTATION_LW, zorder=5)
    ax.plot([x0, x0], [y - cap, y + cap], color=color, linewidth=ANNOTATION_LW, zorder=5)
    ax.plot([x1, x1], [y - cap, y + cap], color=color, linewidth=ANNOTATION_LW, zorder=5)
...
def draw_vertical_bracket(ax, x, y0, y1, *, label=None, color=ANNOTATION_COLOR):
    cap = px_to_data(9)
    ax.plot([x, x], [y0, y1], color=color, linewidth=ANNOTATION_LW, zorder=5)
    ax.plot([x - cap, x + cap], [y0, y0], color=color, linewidth=ANNOTATION_LW, zorder=5)
    ax.plot([x - cap, x + cap], [y1, y1], color=color, linewidth=ANNOTATION_LW, zorder=5)
    if label is not None:
        ax.text(
            x + px_to_data(12),
            (y0 + y1) / 2,
...
OUTPUT_PATH = Path('/home/jake/Developer/MA/PIPELINE/Electrical-and-Computer-Engineering/2-Build-Lessons/4-Image-Generation/1-Outputs/Continuous-Time-Signal-Processing/1.1--continuous-time-signal-basics-Images/images/l006-s001-te-section-001.png')
...
def main():
    ...
    t = np.array([-1, -1, 0, 2, 4], dtype=float)
    x = np.array([0, 1, 3, 1, 0], dtype=float)
    plot_signal(ax, t, x)
    draw_dotted_guide(ax, [-1, 4], [2, 2])
    ax.text(4.14, 2.07, r"$C$", fontsize=ANNOTATION_SIZE, ha="left", va="bottom", color=ANNOTATION_COLOR)
    draw_vertical_bracket(ax, 4.35, 2, 3, label=r"$A$")
    draw_horizontal_bracket(ax, -1, 4, -0.70, label=r"$D$")
