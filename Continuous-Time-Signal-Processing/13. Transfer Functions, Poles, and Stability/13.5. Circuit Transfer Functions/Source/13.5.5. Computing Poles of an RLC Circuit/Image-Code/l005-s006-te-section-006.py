...
def add_legend_panel(ax, *, anchor_xy):
    x0, y0 = anchor_xy
    panel_style = dict(
    ...
    ax.plot(
        ...
        y0 - 0.28,
        marker="o",
        markersize=POLE_MARKER_SIZE * 0.78,
        markerfacecolor=SIGNAL_COLOR,
        markeredgecolor=SIGNAL_COLOR,
        linestyle="None",
        zorder=10,
...
def main():
    ...
    add_region_labels(ax, stable_xy=(-3.85, 2.7), unstable_xy=(1.15, 2.7), boundary_xy=(0.18, 1.0))
    add_legend_panel(ax, anchor_xy=(0.55, -2.95))

    draw_double_pole(ax, -1.0, 0.0)
    draw_pole(ax, -2.0, np.sqrt(6.0))
    draw_pole(ax, -2.0, -np.sqrt(6.0))

    out = Path(
        "/home/jake/Developer/MA/PIPELINE/Electrical-and-Computer-Engineering/2-Build-Lessons/4-Image-Generation/1-Outputs/Continuous-Time-Signal-Processing/13.5--circuit-transfer-functions-Images/images/l005-s006-te-section-006.png"
    )
