...
OUTPUT_PATH = Path(
    "/home/jake/Developer/MA/PIPELINE/Electrical-and-Computer-Engineering/2-Build-Lessons/4-Image-Generation/1-Outputs/Continuous-Time-Signal-Processing/13.3--stability-of-lti-systems-Images/images/l003-s003-te-section-003.png"
)
...
def main():
    ...
    ax.axvspan(xlim[0], 0, color=SIGNAL_COLOR, alpha=0.07, zorder=0)
    ax.text(-2.8, 2.22, r"$\mathrm{LHP}$", fontsize=TOP_LABEL_SIZE * 0.82, ha="left", va="center", color=SIGNAL_COLOR, zorder=2)
    ax.text(1.75, 2.22, r"$\mathrm{RHP}$", fontsize=TOP_LABEL_SIZE * 0.82, ha="left", va="center", color=ANNOTATION_COLOR, zorder=2)
    ax.text(-3.0, -2.45, r"left-half-plane poles satisfy $\Re\{s\}<0$", fontsize=ANNOTATION_SIZE, ha="left", va="center", color=ANNOTATION_COLOR, zorder=2)
    ...
    add_pole(ax, 2, 0, r"$2$", dx=0.16, dy=-0.34, ha="left", va="top")
    add_pole(ax, 0, 2, r"$j2$", dx=0.16, dy=0.18, ha="left", va="bottom")
