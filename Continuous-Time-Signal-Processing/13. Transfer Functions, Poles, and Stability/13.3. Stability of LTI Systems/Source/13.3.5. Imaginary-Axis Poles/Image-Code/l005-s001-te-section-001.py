...
NOTE_SIZE = 31.0 * 72 / CANONICAL_DPI
...
def setup_s_plane_axes(ax, *, xlim, ylim, xticks, yticks):
    ...
    ax.text(0, y_axis_end + Y_AXIS_LABEL_Y_PAD, r"$\Im\{s\}$",
            fontsize=AXIS_LABEL_SIZE, ha="center", va="bottom",
            color=LABEL_COLOR, clip_on=False)
...
def main():
    out_path = Path(
        ...
        "13.3--stability-of-lti-systems-Images/images/l005-s001-te-section-001.png"
    )

    xlim = (-2.6, 2.8)
    ylim = (-2.3, 2.3)
    fig, ax = make_ct_signal_figure(xlim, ylim)
    setup_s_plane_axes(ax, xlim=xlim, ylim=ylim, xticks=[-2, -1, 0, 1, 2], yticks=[-2, -1, 0, 1, 2])

    ax.axvspan(0, xlim[1], color=SIGNAL_COLOR, alpha=0.09, zorder=0)
    ax.axvline(0, color=AXIS_COLOR, linewidth=AXIS_LW * 0.55, alpha=0.7, zorder=2)

    ax.text(-1.55, 1.45, r"$\Re\{s\}<0$", fontsize=NOTE_SIZE, color=LABEL_COLOR,
            ha="center", va="center", zorder=3)
    ax.text(1.35, 1.45, r"$\Re\{s\}>0$", fontsize=NOTE_SIZE, color=LABEL_COLOR,
            ha="center", va="center", zorder=3)
    ax.text(0.0, -1.55, r"$\Re\{s\}=0$", fontsize=NOTE_SIZE, color=LABEL_COLOR,
            ha="center", va="center", zorder=3)

    pole_marker(ax, -1, 0, r"$s=-1$", dx=0.0, dy=0.24)
    pole_marker(ax, 1, 0, r"$s=1$", dx=0.0, dy=0.24)
    pole_marker(ax, 0, 2, r"$j2$", dx=0.20, dy=0.08)
    pole_marker(ax, 0, -2, r"$-j2$", dx=0.28, dy=-0.36)
    pole_marker(ax, 0, 0, r"$s=0$", dx=0.18, dy=0.18)
