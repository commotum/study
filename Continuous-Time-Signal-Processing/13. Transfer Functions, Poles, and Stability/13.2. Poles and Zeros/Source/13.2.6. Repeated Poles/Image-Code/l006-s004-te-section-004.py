...
def draw_repeated_pole(ax, x, y, *, label=r"$m=2$"):
    ...
    ax.text(
        x + POINT_LABEL_OFFSET[0],
        y + POINT_LABEL_OFFSET[1],
        label,
...
def draw_simple_pole(ax, x, y, *, label=r"$m=1$"):
    draw_pole(ax, x, y)
    ax.text(
        x + POINT_LABEL_OFFSET[0],
        y + POINT_LABEL_OFFSET[1],
...
def main():
    ...
    draw_repeated_pole(ax, -1.0, 0.0, label=r"$m=2$")
    draw_simple_pole(ax, -4.0, 0.0, label=r"$m=1$")

    save_figure(
        fig,
        Path(
            "/home/jake/Developer/MA/PIPELINE/Electrical-and-Computer-Engineering/2-Build-Lessons/4-Image-Generation/1-Outputs/Continuous-Time-Signal-Processing/13.2--poles-and-zeros-Images/images/l006-s004-te-section-004.png"
        ),
    )
