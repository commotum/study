...
ax.axvspan(-0.4, 0.0, ymin=0.62, ymax=0.82, color="#b0b0b0", alpha=0.28, zorder=1)
ax.axvspan(0.0, 1.0, ymin=0.38, ymax=0.56, color=SIGNAL_COLOR, alpha=0.16, zorder=1)
ax.axvspan(1.0, 2.0, ymin=0.18, ymax=0.36, color=SIGNAL_COLOR, alpha=0.16, zorder=1)
ax.axvspan(2.0, 3.4, ymin=0.62, ymax=0.82, color="#b0b0b0", alpha=0.28, zorder=1)

for x in [0.0, 1.0, 2.0, 3.0]:
    ax.plot([x, x], [-0.10, 0.92], color=GUIDE_COLOR, linewidth=1.0, linestyle=(0, (1.1, 2.4)), zorder=2)

ax.text(0.02, 0.72, '$y(t)=0$', fontsize=16, ha="center", va="center", color=LABEL_COLOR)
ax.text((0.0 + 1.0) / 2.0, 0.47, '$t^3/6$', fontsize=16, ha="center", va="center", color=LABEL_COLOR)
ax.text((1.0 + 2.0) / 2.0, 0.27, '$1/6$', fontsize=16, ha="center", va="center", color=LABEL_COLOR)
ax.text(3.18, 0.72, '$y(t)=0$', fontsize=16, ha="center", va="center", color=LABEL_COLOR)

ax.text(0.0, 0.57, '$0\\leqq t<1$', fontsize=14, ha="left", va="bottom", color=ANNOTATION_COLOR)
ax.text(1.0, 0.37, '$1\\leqq t<2$', fontsize=14, ha="left", va="bottom", color=ANNOTATION_COLOR)
ax.text(0.02, 0.84, '$t<0$', fontsize=14, ha="center", va="bottom", color=ANNOTATION_COLOR)
ax.text(3.18, 0.84, '$t>3$', fontsize=14, ha="center", va="bottom", color=ANNOTATION_COLOR)
ax.text((-0.4 + 3.4) / 2.0, 0.96, r"assemble into $y(t)$", fontsize=15, ha="center", va="top", color=LABEL_COLOR)

Path('/home/jake/Developer/MA/PIPELINE/Electrical-and-Computer-Engineering/2-Build-Lessons/4-Image-Generation/1-Outputs/Continuous-Time-Signal-Processing/7.4--direct-convolution-for-lti-systems-Images/images/l004-s006-te-section-006.png').parent.mkdir(parents=True, exist_ok=True)
plt.tight_layout()
fig.savefig('/home/jake/Developer/MA/PIPELINE/Electrical-and-Computer-Engineering/2-Build-Lessons/4-Image-Generation/1-Outputs/Continuous-Time-Signal-Processing/7.4--direct-convolution-for-lti-systems-Images/images/l004-s006-te-section-006.png', bbox_inches="tight", facecolor="white")
plt.close(fig)
