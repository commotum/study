...
def main():
    ...
    # Right-triangle product region.
    x0, x1, h = 0.0, 2.0, 2.6
    ax.fill([x0, x1, x1], [0.0, 0.0, h], color=SIGNAL_COLOR, alpha=0.18, zorder=3)
    ax.plot([x0, x1, x1, x0], [0.0, 0.0, h, 0.0], color=SIGNAL_COLOR, lw=SIGNAL_LW, zorder=4)
    ax.text((x0 + x1) / 2 + 0.18, h * 0.64, r"$x(	au)h(t-	au)$", fontsize=15,
            ha="center", va="center", color=SIGNAL_COLOR)

    draw_duration_bracket(ax, x0, x1, y=-0.40, label=r"$t$")
    draw_amplitude_bracket(ax, x1 + 0.30, 0.0, h)
    ax.text(x1 + 0.50, h / 2, '$2t$', fontsize=16, ha="left", va="center", color=LABEL_COLOR)
    ax.text((x0 + x1) / 2, -0.12, '0 <= t <= 1', fontsize=15, ha="center", va="top", color=LABEL_COLOR)
    plt.tight_layout()
    Path('/home/jake/Developer/MA/PIPELINE/Electrical-and-Computer-Engineering/2-Build-Lessons/4-Image-Generation/1-Outputs/Continuous-Time-Signal-Processing/7.4--direct-convolution-for-lti-systems-Images/images/l005-s003-te-section-003.png').parent.mkdir(parents=True, exist_ok=True)
    fig.savefig('/home/jake/Developer/MA/PIPELINE/Electrical-and-Computer-Engineering/2-Build-Lessons/4-Image-Generation/1-Outputs/Continuous-Time-Signal-Processing/7.4--direct-convolution-for-lti-systems-Images/images/l005-s003-te-section-003.png', bbox_inches="tight", facecolor="white")
    plt.close(fig)
