...
def main():
    ...
    # Constant-height rectangular product region.
    x0, x1, h = 0.0, 2.0, 2.0
    ax.fill([x0, x1, x1, x0], [0.0, 0.0, h, h], color=SIGNAL_COLOR, alpha=0.18, zorder=3)
    ax.plot([x0, x1, x1, x0, x0], [0.0, 0.0, h, h, 0.0], color=SIGNAL_COLOR, lw=SIGNAL_LW, zorder=4)
    ax.text((x0 + x1) / 2, h * 0.58, r"$x(	au)h(t-	au)$", fontsize=15,
            ha="center", va="center", color=SIGNAL_COLOR)

    draw_duration_bracket(ax, x0, x1, y=-0.40, label='$2-t$')
    draw_amplitude_bracket(ax, x1 + 0.28, 0.0, h)
    ax.text(x1 + 0.46, h / 2, '$2$', fontsize=16, ha="left", va="center", color=LABEL_COLOR)
    ax.text((x0 + x1) / 2, -0.12, '1 <= t <= 2', fontsize=15, ha="center", va="top", color=LABEL_COLOR)
    plt.tight_layout()
    Path('/home/jake/Developer/MA/PIPELINE/Electrical-and-Computer-Engineering/2-Build-Lessons/4-Image-Generation/1-Outputs/Continuous-Time-Signal-Processing/7.4--direct-convolution-for-lti-systems-Images/images/l005-s002-te-section-002.png').parent.mkdir(parents=True, exist_ok=True)
    fig.savefig('/home/jake/Developer/MA/PIPELINE/Electrical-and-Computer-Engineering/2-Build-Lessons/4-Image-Generation/1-Outputs/Continuous-Time-Signal-Processing/7.4--direct-convolution-for-lti-systems-Images/images/l005-s002-te-section-002.png', bbox_inches="tight", facecolor="white")
    plt.close(fig)
