...
def main():
    fig, ax = make_figure(square=False)
    ax.set_xlim(-2.25, 4.75)
    ax.set_ylim(0.0, 1.18)
    ax.set_aspect("auto")
    for spine in ax.spines.values():
        spine.set_visible(False)
    ax.set_xticks([-2, -1, 0, 2, 4])
    ax.set_yticks([])
    ax.tick_params(left=False, bottom=False, labelleft=False, labelbottom=False)
    ax.quiver(-2.25, 0.10, 7.0, 0, **AXIS_ARROW_KW)
    for t in [-2, -1, 0, 2, 4]:
        ax.plot([t, t], [0.06, 0.14], color=AXIS_COLOR, lw=TICK_LW, zorder=5)
        ax.text(t, -0.02, math_label(t), fontsize=TICK_LABEL_SIZE,
                ha="center", va="top", color=TICK_LABEL_COLOR, zorder=6)
    ax.text(4.87, 0.10, r"$t$", fontsize=AXIS_LABEL_SIZE,
            ha="left", va="center", color=LABEL_COLOR, clip_on=False)
    ax.text((2.5) / 2, 1.03, '$A_i(t) \\rightarrow y(t)$', fontsize=14,
            ha="center", va="bottom", color=LABEL_COLOR)
    for start, end, label, alpha in [[-2.0, -1.0, "zero overlap", 0.08], [-1.0, 0.0, "$A_1(t)=2(t+1)$", 0.16], [0.0, 2.0, "$A_2(t)=2$", 0.16], [2.0, 4.0, "$A_3(t)=4-t$", 0.16], [4.0, 4.65, "zero overlap", 0.08]]:
        ax.axvspan(start, end, ymin=0.18, ymax=0.74, color=SIGNAL_COLOR, alpha=alpha, zorder=1)
        ax.text((start + end) / 2, 0.46, label, fontsize=12,
                ha="center", va="center", color=LABEL_COLOR)
        ax.plot([start, start], [0.18, 0.74], color=AXIS_COLOR, lw=1.0,
                linestyle=(0, (1.1, 2.4)), zorder=2)
    ax.plot([4.75, 4.75], [0.18, 0.74], color=AXIS_COLOR, lw=1.0,
            linestyle=(0, (1.1, 2.4)), zorder=2)
    Path('/home/jake/Developer/MA/PIPELINE/Electrical-and-Computer-Engineering/2-Build-Lessons/4-Image-Generation/1-Outputs/Continuous-Time-Signal-Processing/7.4--direct-convolution-for-lti-systems-Images/images/l005-s006-qg-q001-q001.png').parent.mkdir(parents=True, exist_ok=True)
    plt.tight_layout()
    fig.savefig('/home/jake/Developer/MA/PIPELINE/Electrical-and-Computer-Engineering/2-Build-Lessons/4-Image-Generation/1-Outputs/Continuous-Time-Signal-Processing/7.4--direct-convolution-for-lti-systems-Images/images/l005-s006-qg-q001-q001.png', bbox_inches="tight", facecolor="white")
    plt.close(fig)
