...
LESSON_INDEX = 5
LESSON_TITLE = 'Using Geometry for Piecewise Convolution'
...
def main():
    ...
    # Composite product region split into a rectangle and a triangle.
    rect_x0, rect_x1, rect_h = 0.0, 1.0, 2.0
    tri_x0, tri_x1, tri_h = 1.0, 2.55, 2.0
    ax.fill([rect_x0, rect_x1, rect_x1, rect_x0], [0.0, 0.0, rect_h, rect_h], color=SIGNAL_COLOR, alpha=0.18, zorder=3)
    ax.plot([rect_x0, rect_x1, rect_x1, rect_x0, rect_x0], [0.0, 0.0, rect_h, rect_h, 0.0], color=SIGNAL_COLOR, lw=SIGNAL_LW, zorder=4)
    ax.fill([tri_x0, tri_x1, tri_x1], [0.0, 0.0, tri_h], color=SIGNAL_COLOR, alpha=0.18, zorder=3)
    ax.plot([tri_x0, tri_x1, tri_x1, tri_x0], [0.0, 0.0, tri_h, 0.0], color=SIGNAL_COLOR, lw=SIGNAL_LW, zorder=4)
    ax.text(1.18, max(rect_h, tri_h) * 0.68, r"$x(	au)h(t-	au)$", fontsize=15,
            ha="center", va="center", color=SIGNAL_COLOR)
    draw_dotted_guide(ax, [1.0, 1.0], [0.0, rect_h + 0.25])

    draw_duration_bracket(ax, rect_x0, rect_x1, y=-0.40, label=r"$1$")
    draw_duration_bracket(ax, tri_x0, tri_x1, y=-0.40, label='$t-1$')
    ax.text((rect_x0 + rect_x1) / 2, rect_h / 2, '$2$', fontsize=16, ha="center", va="center", color=LABEL_COLOR)
    ax.text((tri_x0 + tri_x1) / 2 + 0.18, tri_h * 0.62, '$t-1$', fontsize=16, ha="center", va="center", color=LABEL_COLOR)
    ax.text((rect_x0 + tri_x1) / 2, -0.12, '1 <= t <= 2', fontsize=15, ha="center", va="top", color=LABEL_COLOR)
    plt.tight_layout()
    Path('/home/jake/Developer/MA/PIPELINE/Electrical-and-Computer-Engineering/2-Build-Lessons/4-Image-Generation/1-Outputs/Continuous-Time-Signal-Processing/7.4--direct-convolution-for-lti-systems-Images/images/l005-s005-te-section-005.png').parent.mkdir(parents=True, exist_ok=True)
    fig.savefig('/home/jake/Developer/MA/PIPELINE/Electrical-and-Computer-Engineering/2-Build-Lessons/4-Image-Generation/1-Outputs/Continuous-Time-Signal-Processing/7.4--direct-convolution-for-lti-systems-Images/images/l005-s005-te-section-005.png', bbox_inches="tight", facecolor="white")
    plt.close(fig)
