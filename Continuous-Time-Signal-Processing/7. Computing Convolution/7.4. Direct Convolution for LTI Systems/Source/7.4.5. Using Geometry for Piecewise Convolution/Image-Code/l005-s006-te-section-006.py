...
def main():
    ...
    ax.set_yticks([])
    ax.tick_params(left=False, bottom=False, labelleft=False, labelbottom=False)
    ax.quiver(-0.25, 0.10, 3.6, 0, **AXIS_ARROW_KW)
    for t in [0, 1, 2, 3]:
    ...
    ax.text(3.47, 0.10, r"$t$", fontsize=AXIS_LABEL_SIZE,
            ha="left", va="center", color=LABEL_COLOR, clip_on=False)
    ax.text((3.1) / 2, 1.03, '$A_i(t) \\rightarrow y(t)$', fontsize=14,
            ha="center", va="bottom", color=LABEL_COLOR)
    for start, end, label, alpha in [[-0.2, 0.0, "zero overlap", 0.08], [0.0, 1.0, "$A_1(t)=3t$", 0.16], [1.0, 2.0, "$A_2(t)=t^2$", 0.16], [2.0, 3.0, "$A_3(t)=4-t$", 0.16], [3.0, 3.3, "zero overlap", 0.08]]:
    ...
    ax.plot([3.35, 3.35], [0.18, 0.74], color=AXIS_COLOR, lw=1.0,
            linestyle=(0, (1.1, 2.4)), zorder=2)
    Path('/home/jake/Developer/MA/PIPELINE/Electrical-and-Computer-Engineering/2-Build-Lessons/4-Image-Generation/1-Outputs/Continuous-Time-Signal-Processing/7.4--direct-convolution-for-lti-systems-Images/images/l005-s006-te-section-006.png').parent.mkdir(parents=True, exist_ok=True)
    plt.tight_layout()
    fig.savefig('/home/jake/Developer/MA/PIPELINE/Electrical-and-Computer-Engineering/2-Build-Lessons/4-Image-Generation/1-Outputs/Continuous-Time-Signal-Processing/7.4--direct-convolution-for-lti-systems-Images/images/l005-s006-te-section-006.png', bbox_inches="tight", facecolor="white")
    plt.close(fig)
