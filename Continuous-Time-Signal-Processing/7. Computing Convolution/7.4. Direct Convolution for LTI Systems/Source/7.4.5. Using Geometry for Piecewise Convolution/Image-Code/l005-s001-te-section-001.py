...
def main():
    ...
    # Overlap-support schematic for a single fixed t.
    x0, x1 = 1.05, 2.70
    support_x0, support_x1 = 0.00, 3.60
    support_h0, support_h1 = 0.95, 3.85
    product_h = 0.55

    ax.plot([support_x0, support_x1], [1.62, 1.62], color=SIGNAL_COLOR, lw=SIGNAL_LW, zorder=4)
    ax.plot([support_h0, support_h1], [1.00, 1.00], color=SIGNAL_COLOR, lw=SIGNAL_LW, zorder=4)
    ax.text(0.05, 1.78, r"$x(	au)$", fontsize=16, ha="left", va="bottom", color=LABEL_COLOR)
    ax.text(0.05, 1.16, r"$h(t-	au)$", fontsize=16, ha="left", va="bottom", color=LABEL_COLOR)

    ax.fill([x0, x1, x1, x0], [0.0, 0.0, product_h, product_h], color=SIGNAL_COLOR, alpha=0.18, zorder=3)
    ax.plot([x0, x1, x1, x0, x0], [0.0, 0.0, product_h, product_h, 0.0], color=SIGNAL_COLOR, lw=SIGNAL_LW, zorder=4)
    ax.text((x0 + x1) / 2, product_h * 0.58, r"$x(	au)h(t-	au)$", fontsize=15,
            ha="center", va="center", color=SIGNAL_COLOR)
    ax.text((x0 + x1) / 2, product_h * 0.12, r"$C$", fontsize=16,
            ha="center", va="bottom", color=LABEL_COLOR)

    draw_duration_bracket(ax, x0, x1, y=-0.40, label=r"$b(t)-a(t)$")
    ax.text(x0, -0.12, r"$a(t)$", fontsize=TICK_LABEL_SIZE, ha="center", va="top", color=TICK_LABEL_COLOR)
    ax.text(x1, -0.12, r"$b(t)$", fontsize=TICK_LABEL_SIZE, ha="center", va="top", color=TICK_LABEL_COLOR)
    plt.tight_layout()
    Path('/home/jake/Developer/MA/PIPELINE/Electrical-and-Computer-Engineering/2-Build-Lessons/4-Image-Generation/1-Outputs/Continuous-Time-Signal-Processing/7.4--direct-convolution-for-lti-systems-Images/images/l005-s001-te-section-001.png').parent.mkdir(parents=True, exist_ok=True)
    fig.savefig('/home/jake/Developer/MA/PIPELINE/Electrical-and-Computer-Engineering/2-Build-Lessons/4-Image-Generation/1-Outputs/Continuous-Time-Signal-Processing/7.4--direct-convolution-for-lti-systems-Images/images/l005-s001-te-section-001.png', bbox_inches="tight", facecolor="white")
    plt.close(fig)
