...
def main():
    ...
    ax.set_yticks([])
    ax.tick_params(left=False, bottom=False, labelleft=False, labelbottom=False)
    ax.quiver(0.0, 0.10, 4.55, 0, **AXIS_ARROW_KW)
    for t in [1, 2, 4]:
    ...
    ax.text(4.67, 0.10, r"$t$", fontsize=AXIS_LABEL_SIZE,
            ha="left", va="center", color=LABEL_COLOR, clip_on=False)
    ax.text((4.55) / 2, 1.03, '$A_i(t) \\rightarrow y(t)$', fontsize=14,
            ha="center", va="bottom", color=LABEL_COLOR)
    for start, end, label, alpha in [[0.0, 1.0, "zero overlap", 0.08], [1.0, 2.0, "$A_1(t)=t-1$", 0.16], [2.0, 4.0, "$A_2(t)=(4-t)/2$", 0.16], [4.0, 4.5, "zero overlap", 0.08]]:
    ...
    ax.plot([4.55, 4.55], [0.18, 0.74], color=AXIS_COLOR, lw=1.0,
            linestyle=(0, (1.1, 2.4)), zorder=2)
    Path('/home/jake/Developer/MA/PIPELINE/Electrical-and-Computer-Engineering/2-Build-Lessons/4-Image-Generation/1-Outputs/Continuous-Time-Signal-Processing/7.4--direct-convolution-for-lti-systems-Images/images/l005-s006-qg-q002-q002.png').parent.mkdir(parents=True, exist_ok=True)
    plt.tight_layout()
    fig.savefig('/home/jake/Developer/MA/PIPELINE/Electrical-and-Computer-Engineering/2-Build-Lessons/4-Image-Generation/1-Outputs/Continuous-Time-Signal-Processing/7.4--direct-convolution-for-lti-systems-Images/images/l005-s006-qg-q002-q002.png', bbox_inches="tight", facecolor="white")
    plt.close(fig)
