...
def main():
    ...
    draw_duration_bracket(ax, x0, x1, y=-0.40, label='$3-t$')
    draw_amplitude_bracket(ax, x0 - 0.28, 0.0, h_left)
    draw_amplitude_bracket(ax, x1 + 0.28, 0.0, h_right)
    ax.text(x0 - 0.48, h_left / 2, '$1$', fontsize=16, ha="right", va="center", color=LABEL_COLOR)
    ax.text(x1 + 0.48, h_right / 2, '$3$', fontsize=16, ha="left", va="center", color=LABEL_COLOR)
    ax.text((x0 + x1) / 2, -0.12, '2 <= t <= 3', fontsize=15, ha="center", va="top", color=LABEL_COLOR)
    plt.tight_layout()
    Path('/home/jake/Developer/MA/PIPELINE/Electrical-and-Computer-Engineering/2-Build-Lessons/4-Image-Generation/1-Outputs/Continuous-Time-Signal-Processing/7.4--direct-convolution-for-lti-systems-Images/images/l005-s004-qg-q002-q002.png').parent.mkdir(parents=True, exist_ok=True)
    fig.savefig('/home/jake/Developer/MA/PIPELINE/Electrical-and-Computer-Engineering/2-Build-Lessons/4-Image-Generation/1-Outputs/Continuous-Time-Signal-Processing/7.4--direct-convolution-for-lti-systems-Images/images/l005-s004-qg-q002-q002.png', bbox_inches="tight", facecolor="white")
    plt.close(fig)
