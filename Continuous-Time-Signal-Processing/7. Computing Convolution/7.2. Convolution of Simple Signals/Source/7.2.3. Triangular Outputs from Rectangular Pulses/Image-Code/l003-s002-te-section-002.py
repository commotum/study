...
def main():
    ...
    image_output_path = Path(r"/home/jake/Developer/MA/PIPELINE/Electrical-and-Computer-Engineering/2-Build-Lessons/4-Image-Generation/1-Outputs/Continuous-Time-Signal-Processing/7.2--convolution-of-simple-signals-Images/images/l003-s002-te-section-002.png")
    image_output_path.parent.mkdir(parents=True, exist_ok=True)

    fig = plt.figure(figsize=(11.2, 8.8), dpi=DPI)
    gs = fig.add_gridspec(2, 3, height_ratios=[1.0, 1.0], hspace=0.50, wspace=0.34)

    def draw_contact_panel(ax, t_shift, title):
        ...
        draw_pulse_box(ax, 0, 2, height=1, baseline=0, fill_alpha=0.10)
        draw_pulse_box(ax, t_shift - 2, t_shift, height=1, baseline=0, fill_alpha=0.05)
        overlap_left = max(0, t_shift - 2)
        overlap_right = min(2, t_shift)
        if overlap_right <= overlap_left:
            overlap_right = overlap_left + 0.05
        ax.axvspan(overlap_left, overlap_right, color=SIGNAL_COLOR, alpha=0.18, zorder=1)
        ax.text(0.10, 1.16, r"$x(\tau)$", fontsize=16, color=LABEL_COLOR, ha="left", va="bottom")
        ax.text(t_shift - 1.28, 1.16, r"$h(t-\tau)$", fontsize=16, color=LABEL_COLOR, ha="left", va="bottom")
        ax.text(0, -0.17, r"$0$", fontsize=15, color=TICK_LABEL_COLOR, ha="center", va="top")
        ax.text(2, -0.17, r"$2$", fontsize=15, color=TICK_LABEL_COLOR, ha="center", va="top")
        ax.text(t_shift - 2, -0.17, r"$t-2$", fontsize=15, color=TICK_LABEL_COLOR, ha="center", va="top")
        ax.text(t_shift, -0.17, r"$t$", fontsize=15, color=TICK_LABEL_COLOR, ha="center", va="top")
        ax.text(0.5, 1.03, title, transform=ax.transAxes, fontsize=15, color=LABEL_COLOR, ha="center", va="bottom")
    ...
    draw_contact_panel(ax1, 0, "first contact")
    draw_contact_panel(ax2, 2, "maximum overlap")
    draw_contact_panel(ax3, 4, "last contact")

    draw_timeline_axis(ax4, 0, 4.35, markers=[0, 2, 4], marker_labels=[(0, r"t = 0"), (2, r"t = 2"), (4, r"t = 4")], label_y=-0.22)
    ax4.text(0.10, 0.26, r"zero", fontsize=13, color=LABEL_COLOR, ha="left", va="center")
    ax4.text(1.00, 0.26, r"rising", fontsize=13, color=LABEL_COLOR, ha="center", va="center")
    ax4.text(3.00, 0.26, r"falling", fontsize=13, color=LABEL_COLOR, ha="center", va="center")
    ax4.text(4.18, 0.26, r"zero", fontsize=13, color=LABEL_COLOR, ha="left", va="center")
