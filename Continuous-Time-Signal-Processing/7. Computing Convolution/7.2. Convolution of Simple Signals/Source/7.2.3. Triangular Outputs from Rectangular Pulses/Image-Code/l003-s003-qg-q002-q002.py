...
def main():
    ...
    image_output_path = Path(r"/home/jake/Developer/MA/PIPELINE/Electrical-and-Computer-Engineering/2-Build-Lessons/4-Image-Generation/1-Outputs/Continuous-Time-Signal-Processing/7.2--convolution-of-simple-signals-Images/images/l003-s003-qg-q002-q002.png")
    image_output_path.parent.mkdir(parents=True, exist_ok=True)

    fig = plt.figure(figsize=(9.5, 6.8), dpi=DPI)
    ...
    draw_pulse_box(ax, 0, 4, height=3, baseline=0, fill_alpha=0.10)
    draw_pulse_box(ax, -1.0, 3.0, height=5, baseline=0, fill_alpha=0.05)
    overlap_left = max(0, -1.0)
    overlap_right = min(4, 3.0)
    if overlap_right <= overlap_left:
        overlap_right = overlap_left + 0.05
    ax.axvspan(overlap_left, overlap_right, color=SIGNAL_COLOR, alpha=0.18, zorder=1)
    ax.text(0.08, 3.13, r"$x(\tau)$", fontsize=15, color=LABEL_COLOR, ha="left", va="bottom")
    ax.text(-1.0 - 0.06, 5.13, r"$h(t-\tau)$", fontsize=15, color=LABEL_COLOR, ha="left", va="bottom")
    ax.text((overlap_left + overlap_right) / 2, 0.20, r"overlap length t", fontsize=13, color=ANNOTATION_COLOR, ha="center", va="bottom")
    ax.text((overlap_left + overlap_right) / 2, 5.26, r"height product 3 times 5", fontsize=14.5, color=ANNOTATION_COLOR, ha="center", va="bottom")
    ax.text(0, -0.19, r"$0$", fontsize=15, color=TICK_LABEL_COLOR, ha="center", va="top")
    ax.text(4, -0.19, r"$4$", fontsize=15, color=TICK_LABEL_COLOR, ha="center", va="top")
    ax.text(-1.0, -0.19, r"$t-4$", fontsize=15, color=TICK_LABEL_COLOR, ha="center", va="top")
    ax.text(3.0, -0.19, r"$t$", fontsize=15, color=TICK_LABEL_COLOR, ha="center", va="top")
    ax.text(0.5, 1.03, r"Rising overlap area for height-3 and height-5 pulses", transform=ax.transAxes, fontsize=15, color=LABEL_COLOR, ha="center", va="bottom")
