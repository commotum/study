...
def main():
    ...
    image_output_path = Path(r"/home/jake/Developer/MA/PIPELINE/Electrical-and-Computer-Engineering/2-Build-Lessons/4-Image-Generation/1-Outputs/Continuous-Time-Signal-Processing/7.2--convolution-of-simple-signals-Images/images/l003-s004-qg-q002-q002.png")
    ...
    draw_pulse_box(ax_main, 0, 5, height=2, baseline=0, fill_alpha=0.10)
    draw_pulse_box(ax_main, 3, 8, height=4, baseline=0, fill_alpha=0.05)
    overlap_left = max(0, 3)
    overlap_right = min(5, 8)
    ...
    ax_main.text(3 - 0.06, 4.13, r"$h(t-\tau)$", fontsize=15, color=LABEL_COLOR, ha="left", va="bottom")
    ax_main.text((overlap_left + overlap_right) / 2, 0.20, r"shrinking overlap length 10 - t", fontsize=13, color=ANNOTATION_COLOR, ha="center", va="bottom")
    ax_main.text((overlap_left + overlap_right) / 2, 4.26, r"height product 2 times 4", fontsize=14.5, color=ANNOTATION_COLOR, ha="center", va="bottom")
    ax_main.text(0, -0.19, r"$0$", fontsize=15, color=TICK_LABEL_COLOR, ha="center", va="top")
    ax_main.text(5, -0.19, r"$5$", fontsize=15, color=TICK_LABEL_COLOR, ha="center", va="top")
    ax_main.text(3, -0.19, r"$t-5$", fontsize=15, color=TICK_LABEL_COLOR, ha="center", va="top")
    ax_main.text(8, -0.19, r"$t$", fontsize=15, color=TICK_LABEL_COLOR, ha="center", va="top")
    ax_main.text(0.5, 1.03, r"Falling overlap area for height-2 and height-4 pulses", transform=ax_main.transAxes, fontsize=15, color=LABEL_COLOR, ha="center", va="bottom")
    ...
    draw_line_segment(ax_branch, 5, 20, 10, 0)
    ax_branch.text(5.1, 20.15, r"falling branch", fontsize=13, color=LABEL_COLOR, ha="left", va="bottom")
