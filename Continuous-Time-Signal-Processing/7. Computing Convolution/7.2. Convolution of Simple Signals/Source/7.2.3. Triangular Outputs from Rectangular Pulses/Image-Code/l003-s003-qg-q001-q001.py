...
def main():
    ...
    image_output_path = Path(r"/home/jake/Developer/MA/PIPELINE/Electrical-and-Computer-Engineering/2-Build-Lessons/4-Image-Generation/1-Outputs/Continuous-Time-Signal-Processing/7.2--convolution-of-simple-signals-Images/images/l003-s003-qg-q001-q001.png")
    ...
    draw_pulse_box(ax, 0, 3, height=2, baseline=0, fill_alpha=0.10)
    draw_pulse_box(ax, -1.0, 2.0, height=4, baseline=0, fill_alpha=0.05)
    overlap_left = max(0, -1.0)
    overlap_right = min(3, 2.0)
    if overlap_right <= overlap_left:
        overlap_right = overlap_left + 0.05
    ax.axvspan(overlap_left, overlap_right, color=SIGNAL_COLOR, alpha=0.18, zorder=1)
    ax.text(0.08, 2.13, r"$x(\tau)$", fontsize=15, color=LABEL_COLOR, ha="left", va="bottom")
    ax.text(-1.0 - 0.06, 4.13, r"$h(t-\tau)$", fontsize=15, color=LABEL_COLOR, ha="left", va="bottom")
    ax.text((overlap_left + overlap_right) / 2, 0.20, r"overlap length t", fontsize=13, color=ANNOTATION_COLOR, ha="center", va="bottom")
    ax.text((overlap_left + overlap_right) / 2, 4.26, r"height product 2 times 4", fontsize=14.5, color=ANNOTATION_COLOR, ha="center", va="bottom")
    ax.text(0, -0.19, r"$0$", fontsize=15, color=TICK_LABEL_COLOR, ha="center", va="top")
    ax.text(3, -0.19, r"$3$", fontsize=15, color=TICK_LABEL_COLOR, ha="center", va="top")
    ax.text(-1.0, -0.19, r"$t-3$", fontsize=15, color=TICK_LABEL_COLOR, ha="center", va="top")
    ax.text(2.0, -0.19, r"$t$", fontsize=15, color=TICK_LABEL_COLOR, ha="center", va="top")
    ax.text(0.5, 1.03, r"Rising overlap area for height-2 and height-4 pulses", transform=ax.transAxes, fontsize=15, color=LABEL_COLOR, ha="center", va="bottom")
