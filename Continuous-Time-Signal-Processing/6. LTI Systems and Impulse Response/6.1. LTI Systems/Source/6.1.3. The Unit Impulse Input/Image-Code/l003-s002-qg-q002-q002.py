"""Question-image showing candidate signals in alternate order."""
...
def draw_signal_variant(ax, kind, *, label):
    ...
    if kind == "impulse-scale-3":
        draw_unit_impulse(ax, 0, amp=1.5)
        ax.text(0.0, 1.38, r"3", fontsize=TICK_LABEL_SIZE * 0.85, color=LABEL_COLOR, ha="left", va="bottom")
    elif kind == "step":
        draw_step(ax)
    elif kind == "pulse":
        draw_rect_pulse(ax)
    elif kind == "impulse-adv":
        draw_unit_impulse(ax, -1, amp=1.0)
    elif kind == "impulse":
...
panel_defs = [
    ("A", r"$3\delta(t)$", "impulse-scale-3"),
    ("B", r"$u(t)$", "step"),
    ("C", "rectangular pulse", "pulse"),
    ("D", r"$\delta(t+1)$", "impulse-adv"),
    ("E", r"$\delta(t)$", "impulse"),
]
...
panel_width = 0.165
gap = 0.015
for i, (label, symbol, kind) in enumerate(panel_defs):
...
output_path = Path('/home/jake/Developer/MA/PIPELINE/Electrical-and-Computer-Engineering/2-Build-Lessons/4-Image-Generation/1-Outputs/Continuous-Time-Signal-Processing/6.1--lti-systems-Images/images/l003-s002-qg-q002-q002.png')
