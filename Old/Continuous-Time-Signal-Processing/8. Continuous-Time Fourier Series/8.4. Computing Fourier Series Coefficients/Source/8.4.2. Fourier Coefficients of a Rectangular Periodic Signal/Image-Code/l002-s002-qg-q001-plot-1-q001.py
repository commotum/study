...
def plot_signal(ax, t, x, *, lw=SIGNAL_LW, color=SIGNAL_COLOR, zorder=4):
    ax.plot(np.asarray(t), np.asarray(x), color=color, linewidth=lw, solid_capstyle="butt", solid_joinstyle="miter", zorder=zorder)


def draw_duration_bracket(ax, start, end, y=-0.38, *, label=None):
    ax.plot([start, end], [y, y], color=ANNOTATION_COLOR, linewidth=ANNOTATION_LW, zorder=5)
    ax.plot([start, start], [y - BRACKET_CAP_HALF_LEN, y + BRACKET_CAP_HALF_LEN], color=ANNOTATION_COLOR, linewidth=ANNOTATION_LW, zorder=5)
    ax.plot([end, end], [y - BRACKET_CAP_HALF_LEN, y + BRACKET_CAP_HALF_LEN], color=ANNOTATION_COLOR, linewidth=ANNOTATION_LW, zorder=5)
    if label is not None:
        ax.text((start + end) / 2, y - BRACKET_LABEL_GAP, label, fontsize=ANNOTATION_SIZE, ha="center", va="top", color=ANNOTATION_COLOR)


def draw_amplitude_bracket(ax, x_bracket, y0, y1, *, label=None):
    ax.plot([x_bracket, x_bracket], [y0, y1], color=ANNOTATION_COLOR, linewidth=ANNOTATION_LW, zorder=5)
    ax.plot([x_bracket - AMPLITUDE_CAP_HALF_LEN, x_bracket + AMPLITUDE_CAP_HALF_LEN], [y0, y0], color=ANNOTATION_COLOR, linewidth=ANNOTATION_LW, zorder=5)
    ax.plot([x_bracket - AMPLITUDE_CAP_HALF_LEN, x_bracket + AMPLITUDE_CAP_HALF_LEN], [y1, y1], color=ANNOTATION_COLOR, linewidth=ANNOTATION_LW, zorder=5)
    if label is not None:
        ax.text(x_bracket, y1 + px_to_data(14), label, fontsize=ANNOTATION_SIZE, ha="center", va="bottom", color=ANNOTATION_COLOR)
...
def rectangular_periodic_trace(amplitude, width, period, left_pad, right_pad):
    t = np.array([left_pad, 0.0, 0.0, width, width, period, period, period + width, period + width, right_pad], dtype=float)
...
OUTPUT_PATH = Path(
    "/home/jake/Developer/MA/PIPELINE/Electrical-and-Computer-Engineering/2-Build-Lessons/4-Image-Generation/1-Outputs/Continuous-Time-Signal-Processing/8.4--computing-fourier-series-coefficients-Images/images/l002-s002-qg-q001-plot-1-q001.png"
)
