...
def main():
    ...
    image_output_path = Path(r"/home/jake/Developer/MA/PIPELINE/Electrical-and-Computer-Engineering/2-Build-Lessons/4-Image-Generation/1-Outputs/Continuous-Time-Signal-Processing/7.2--convolution-of-simple-signals-Images/images/l003-s004-qg-q001-q001.png")
    image_output_path.parent.mkdir(parents=True, exist_ok=True)

    fig = plt.figure(figsize=(9.6, 7.0), dpi=DPI)
    gs = fig.add_gridspec(2, 1, height_ratios=[1.22, 0.68], hspace=0.30)
    ...
    draw_pulse_box(ax_main, 0, 4, height=3, baseline=0, fill_alpha=0.10)
    draw_pulse_box(ax_main, 2, 6, height=2, baseline=0, fill_alpha=0.05)
    overlap_left = max(0, 2)
    overlap_right = min(4, 6)
    if overlap_right <= overlap_left:
        overlap_right = overlap_left + 0.05
    ax_main.axvspan(overlap_left, overlap_right, color=SIGNAL_COLOR, alpha=0.18, zorder=1)
    ax_main.text(0.08, 3.13, r"$x(\tau)$", fontsize=15, color=LABEL_COLOR, ha="left", va="bottom")
    ax_main.text(2 - 0.06, 2.13, r"$h(t-\tau)$", fontsize=15, color=LABEL_COLOR, ha="left", va="bottom")
    ax_main.text((overlap_left + overlap_right) / 2, 0.20, r"shrinking overlap length 8 - t", fontsize=13, color=ANNOTATION_COLOR, ha="center", va="bottom")
    ax_main.text((overlap_left + overlap_right) / 2, 3.26, r"height product 3 times 2", fontsize=14.5, color=ANNOTATION_COLOR, ha="center", va="bottom")
    ax_main.text(0, -0.19, r"$0$", fontsize=15, color=TICK_LABEL_COLOR, ha="center", va="top")
    ax_main.text(4, -0.19, r"$4$", fontsize=15, color=TICK_LABEL_COLOR, ha="center", va="top")
    ax_main.text(2, -0.19, r"$t-4$", fontsize=15, color=TICK_LABEL_COLOR, ha="center", va="top")
    ax_main.text(6, -0.19, r"$t$", fontsize=15, color=TICK_LABEL_COLOR, ha="center", va="top")
    ax_main.text(0.5, 1.03, r"Falling overlap area for height-3 and height-2 pulses", transform=ax_main.transAxes, fontsize=15, color=LABEL_COLOR, ha="center", va="bottom")
    ...
    draw_line_segment(ax_branch, 4, 12, 8, 0)
    ax_branch.text(4.1, 12.15, r"falling branch", fontsize=13, color=LABEL_COLOR, ha="left", va="bottom")
