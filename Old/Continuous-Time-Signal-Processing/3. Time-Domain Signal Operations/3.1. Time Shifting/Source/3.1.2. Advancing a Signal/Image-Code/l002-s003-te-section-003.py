...
OUTPUT_PATH = Path('/home/jake/Developer/MA/PIPELINE/Electrical-and-Computer-Engineering/2-Build-Lessons/4-Image-Generation/1-Outputs/Continuous-Time-Signal-Processing/3.1--time-shifting-Images/images/l002-s003-te-section-003.png')
...
def build_figure():
    ...
    original_t = [-1, 0, 1, 2, 3, 4, 4, 5]
    original_x = [0, 0, 2, 2, 1, 1, 0, 0]
    advanced_t = [-3, -2, -1, 0, 1, 2, 2, 3]
    advanced_x = [0, 0, 2, 2, 1, 1, 0, 0]
    ...
    mark(ax, 1, 2, '$1$', dx=0.0, dy=0.16, guide=True)

    mark(ax, -1, 2, '$-1$', dx=0.0, dy=0.16, guide=True)

    mark(ax, 0, 0, '$0$', dx=0.0, dy=0.14, guide=False)

    mark(ax, -2, 0, '$-2$', dx=0.0, dy=0.14, guide=False)

    draw_shift_arrow(ax, 1.2, -0.8, 2.45, label='$2$')

    return fig

def main():
    fig = build_figure()
    save_figure(fig, Path('/home/jake/Developer/MA/PIPELINE/Electrical-and-Computer-Engineering/2-Build-Lessons/4-Image-Generation/1-Outputs/Continuous-Time-Signal-Processing/3.1--time-shifting-Images/images/l002-s003-te-section-003.png'))
