...
ANNOTATION_SIZE = 23.0 * 72 / CANONICAL_DPI
...
def add_half_plane_shading_and_labels(ax, xlim, ylim):
...
def render(output_path, poles, zeros, note_text):
    xlim = (-4.4, 2.4)
    ylim = (-2.6, 2.6)
    fig, ax = make_figure(xlim, ylim)
    add_half_plane_shading_and_labels(ax, xlim, ylim)
...
if __name__ == "__main__":
    output_path = Path(
        "/home/jake/Developer/MA/PIPELINE/Electrical-and-Computer-Engineering/2-Build-Lessons/4-Image-Generation/1-Outputs/Continuous-Time-Signal-Processing/13.3--stability-of-lti-systems-Images/images/l002-s004-qg-q001-q001.png"
    )
    poles = np.array([-3.0 + 0.0j, -0.5 + 1.5j, -0.5 - 1.5j])
    zeros = np.array([1.5 + 0.0j, 0.0 - 2.0j])
    render(output_path, poles, zeros, r"BIBO stability depends only on pole real parts.")
