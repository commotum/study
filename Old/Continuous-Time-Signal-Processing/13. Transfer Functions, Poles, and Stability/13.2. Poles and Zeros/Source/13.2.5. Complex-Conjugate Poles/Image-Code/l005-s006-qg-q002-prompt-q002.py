...
def main():
    output_path = Path('/home/jake/Developer/MA/PIPELINE/Electrical-and-Computer-Engineering/2-Build-Lessons/4-Image-Generation/1-Outputs/Continuous-Time-Signal-Processing/13.2--poles-and-zeros-Images/images/l005-s006-qg-q002-prompt-q002.png')
    fig, ax = make_figure((-6.5, 6.5), (-6.5, 6.5))
    setup_axes(
        ax,
        xlim=(-6.5, 6.5),
        ylim=(-6.5, 6.5),
    ...
    draw_zero(ax, -4.5, 2.0, color="#aeb6c2", size=14.0 * 72 / CANONICAL_DPI, edgewidth=4.0 * 72 / CANONICAL_DPI, alpha=0.65)
    draw_pole(ax, 4.2, -2.5, color="#aeb6c2", size=14.5 * 72 / CANONICAL_DPI, edgewidth=4.0 * 72 / CANONICAL_DPI, alpha=0.65)
    draw_pole(ax, 2, 5)
    add_label(ax, 2, 5, r"$(2, 5)$", dx=0.18, dy=0.20, va="bottom")
    draw_pole(ax, 2, -5)
    add_label(ax, 2, -5, r"$(2, -5)$", dx=0.18, dy=-0.36, va="top")
